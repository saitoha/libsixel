...
The [command-length failures around the 2,000-test milestone](test-harness.md#what-happened-when-the-suite-passed-2000-tests) demonstrate both the limits and the value of this flexibility. With 2,047 runnable scripts, the flattened test-path inventory alone occupied about 134 kB, and recipes that expanded whole inventories encountered process-launch limits. Even a mature build system with a reputation for robustness can fail when a project's scale exceeds the assumptions of its standard recipes. Automake's overridable rules and distribution hooks let libsixel repair the affected paths with straightforward local make and shell changes, keeping long lists in dependencies or files and processing paths incrementally. The project retained its tests, parallel scheduler, and build system while removing the problematic expansions.

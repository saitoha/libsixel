...
### Generalizing to four points in RGB space

The grayscale example is an introduction: a palette entry exactly at M is only one special case. Actual colors have three coordinates, red, green, and blue. A and B define a line, and their midpoint M lies on it. The closest **other** palette color P can lie anywhere in RGB space, including off that line. P is chosen by distance to M, not by distance to the segment A–B or by whether it lies between A and B. The endpoint entries themselves are excluded from this search.

For each pair, the geometric question is how close P comes to the hypothetical intermediate color M relative to the spacing of A and B. Define `d = |M−A|²` and `q = |M−P|²`, using squared Euclidean RGB distance. The triangle A–P–B visualizes the arrangement, with M on its base; the rule extracts `q/d` from that geometry. It does not use the triangle's angle, area, or P's projection onto A–B to solve a mixing ratio.

<picture>
  <source media="(max-width: 640px)" srcset="dequantization-figures/rgb-geometry-mobile.svg">
  <img alt="Three RGB palettes share A, B and midpoint M but have different nearest colors P off the A-B line. Squared distance ratios 0.125, 0.703125 and 1.25 give different neighbor weights." src="dequantization-figures/rgb-geometry-wide.svg">
</picture>

*Each panel is a separate palette, not three candidates in a single search. All points have blue=128, so the equal-scale red/green plane preserves actual RGB distances. The crosses are additional palette entries, both farther from M than P. The dashed circle has radius |M−A|. The displayed weights are pair weights before center protection and neighborhood normalization.*

This plane is one valid colored example, not a restriction of the algorithm to two dimensions. In general P can also differ in blue; the corresponding constant-distance boundary around M is a sphere in RGB space. P lying on A–B, or exactly at M, is unnecessary.

There is also a useful limit to what the geometry tells the implementation. Moving P around M while keeping |M−P| unchanged changes the triangle but **does not change this pair's weight**, provided P remains the nearest other entry. For example, P=(160,192,128) and P=(192,160,128) both give `q=5120` and the same weight. Conversely, moving the nearest entry closer or farther changes `q/d`, which may cross a coefficient threshold. This is evidence for whether A and B plausibly represent a missing shade, not a calibrated probability that a particular mixture is correct.

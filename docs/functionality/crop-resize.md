...
| `sixel_helper_scale_image()` | Scales caller-provided byte-addressed pixels into caller-provided storage. When the input's computed depth is not three bytes, the helper first applies `sixel_helper_normalize_pixelformat()` and writes the normalized layout: `RGB888` for `G8`, packed-RGB, alpha-bearing, and float inputs, or unpacked `G8`/`PAL8` for packed grayscale/palette input. The caller must allocate the destination for that effective format. The helper does not update frame dimensions, ownership, palette, colorspace, or transparency metadata. |

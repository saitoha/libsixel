...
- [delta encoding (6delta)](delta-encoding.md), selected by `-Z delta` with `threshold` and `error` suboptions, which compares new pixels with retained display colors and omits suitable repaints using `P2=1`;

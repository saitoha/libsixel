...
The method switch, kernel functions, coordinate expressions, serial and thread-pool passes, SIMD dispatch, and normalization are all implemented in [`src/scale.c`](../../src/scale.c). Focused scale behavior is covered by the C tests under [`tests/processing/scale/`](../../tests/processing/scale/) and CLI method comparisons under [`tests/processing/geometry/`](../../tests/processing/geometry/), while frame-level resize and mask behavior is covered under [`tests/processing/frame/`](../../tests/processing/frame/). The [resampling regression inventory](../testing/resampling-coverage.md) links the exact precision/SIMD/thread intersections and the numeric measurement gates.

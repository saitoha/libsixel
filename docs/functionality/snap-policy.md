...
The current generated byte-palette ordering places cover repair after the final snap; a merge-funded cover replacement or image-derived soft anchor can therefore introduce a value outside the safe set. Cover off is necessary for isolating snap, but is not sufficient for non-gamma clustering or working spaces: later color-space conversion can also undo the final byte snap, as described in the [measurement limitation](#measured-snapshot).
...
**Measurement limitation:** The snapshot below predates the `-M` output-color-space correction in `f1ddead42` and the DIN99d correction in `c26da16e7`. Its non-gamma fixed-point results inspected working-space byte values rather than the exported output RGB values, so the claimed zero-drift result does not establish output-palette reversibility. A current rerun detects unsafe output channels for linear RGB, Oklab, CIELAB, and DIN99d even at `rate=1` with cover off. The final K-means snap currently treats packed clustering coordinates as `RGB888`; later clustering-to-working or working-to-output conversion can undo that grid alignment. Do not rely on the fixed-point guarantee for those paths or use the old DIN99d curves to evaluate the corrected space.

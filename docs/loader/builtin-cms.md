...
| BCMS-05 | Selected source-to-destination intent paths write representative RGB8, CMYK8, CMYK16, and CMYK float32 channel results. | [tests/diagnostics/icc/0005_icc_builtin_device_to_device_intent_paths.t](../../tests/diagnostics/icc/0005_icc_builtin_device_to_device_intent_paths.t) |
...
| BCMS-08 | The builtin facade accepts representative Gray8, RGB8, RGB float32, and RGBA8 source/destination combinations, expands gray to equal RGB components, and preserves copied alpha. | [tests/diagnostics/icc/0006_icc_builtin_pixel_format_matrix.t](../../tests/diagnostics/icc/0006_icc_builtin_pixel_format_matrix.t) |
...
| BCMS-10 | D2B0/B2D0 slot routing accepts the documented nonstandard legacy `mft2` payload and produces the exact identity round trip. | [tests/diagnostics/icc/0008_icc_builtin_d2b_b2d_legacy_lut.t](../../tests/diagnostics/icc/0008_icc_builtin_d2b_b2d_legacy_lut.t) |
| BCMS-11 | PNG cHRM/gAMA synthesis produces fixed linear float samples through the builtin CMS. | [tests/loader/builtin/2040_loader_builtin_png_chrm_gama_float_numeric.t](../../tests/loader/builtin/2040_loader_builtin_png_chrm_gama_float_numeric.t) |
| BCMS-12 | A singular PNG cHRM matrix is rejected while the independently usable gAMA interpretation remains active. | [tests/loader/builtin/2041_loader_builtin_png_singular_chrm_fallback_numeric.t](../../tests/loader/builtin/2041_loader_builtin_png_singular_chrm_fallback_numeric.t) |
| BCMS-13 | Allocation failures while opening optional PNG ICC metadata release all allocations and may follow the documented unmanaged fallback. | [tests/loader/builtin/2035_loader_builtin_png_icc_allocation_failures.t](../../tests/loader/builtin/2035_loader_builtin_png_icc_allocation_failures.t) |
...
The current tests do not establish complete ICC v2/v4 conformance, arbitrary profile-class support, mpet execution, fidelity for non-identity legacy LUT matrices, independent absolute-intent behavior, uniform resource limits, every float-bearing device/Lab path, or performance/quality equivalence across CMS engines. Representative gray reads, CMYK stores, loader-integrated synthesis, and optional-profile allocation failures now have direct owners, but header validation, every object lifetime path, and the full source/destination format product still require more than the selected tests listed here. Those limits remain visible engineering work rather than being inferred away from a passing test suite.

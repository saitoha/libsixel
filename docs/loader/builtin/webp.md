...
Coverage audit note: direct owners now fix representative VP8 and VP8L RGB output, one multi-partition VP8 stream, all three specified ALPH reconstruction filters, both full-frame and subrectangle animation canvases, and animation cancellation boundaries. The suite still does not exhaust every VP8 macroblock/filter/partition mode, VP8L Huffman/transform nesting, ALPH compression/preprocessing combination, animation blend/dispose permutation, or metadata precedence/size-limit combination.

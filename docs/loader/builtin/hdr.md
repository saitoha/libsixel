...
Coverage audit note: profile priority, orientation, all three scanline coding dialects, baseline linear values, exposure, and a typed target are owned. The broader header-metadata and orientation matrices remain intentionally separate from the three small coding-dialect tests.

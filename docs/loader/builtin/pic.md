...
Coverage audit note: the owners now distinguish recognition, packet chaining and overwrites, omitted-channel initialization, raw decode, pure-RLE clipping, all three mixed-RLE count encodings, alpha composition, the ten-packet bound, and mixed-run overrun. Zero dimensions, an eleventh packet, and every multi-channel packet-mask combination remain explicit gaps.

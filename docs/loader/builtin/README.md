...
The diagram-linked tests are pipeline landmarks, not the complete regression suite. The [Builtin Loader Coverage Inventory](../../testing/builtin-loader-coverage.md) lists every shell test in `tests/loader/builtin/`, separates policy-backed contracts from supplementary and defensive cases, and makes additions, deletions, and renames part of the reciprocal `Test-plan:` check.

...
The [libpng Image Loader](../libpng.md) reference documents that adapter's current shared alpha/background policy, float APNG canvas, original-stream validation, and coverage boundaries.
...
The following comparison tables and numerical examples record the earlier adapter used for the measurements below. They are historical observations, not current libpng behavior. The repaired libpng adapter retains sixteen-bit alpha/APNG samples, preserves masks independently of CMS, honors background priority, and blends in linear light; its [current contracts and numerical tests](../libpng.md#test-coverage) supersede the old libpng path limitations listed here. The recorded timing results have not been rerun for the repaired adapter.
...
| PNG-16 | A minimal CgBI stream converts premultiplied BGRA storage into the fixed straight RGB sample. | [tests/loader/builtin/2023_loader_builtin_png_cgbi_numeric.t](../../../tests/loader/builtin/2023_loader_builtin_png_cgbi_numeric.t) |
| PNG-17 | Cancellation before APNG decode and immediately after its first callback both return `SIXEL_INTERRUPTED` without emitting a later frame. | [tests/loader/builtin/2028_loader_builtin_apng_cancel_boundaries.t](../../../tests/loader/builtin/2028_loader_builtin_apng_cancel_boundaries.t) |
| PNG-18 | cHRM/gAMA synthesis converts an RGBA source into fixed linear float samples without reducing it to byte precision. | [tests/loader/builtin/2040_loader_builtin_png_chrm_gama_float_numeric.t](../../../tests/loader/builtin/2040_loader_builtin_png_chrm_gama_float_numeric.t) |
| PNG-19 | A singular cHRM matrix falls back to the independently usable gAMA path and produces the same exact frame as the gAMA-only source. | [tests/loader/builtin/2041_loader_builtin_png_singular_chrm_fallback_numeric.t](../../../tests/loader/builtin/2041_loader_builtin_png_singular_chrm_fallback_numeric.t) |

...
Coverage audit note: direct owners now fix all five scanline filters in one image, all seven Adam7 passes for one indexed/depth combination, one sub-eight-bit Gray16 boundary, CgBI channel/unpremultiplication normalization, APNG cancellation boundaries, and representative `SOURCE`/`OVER`/`BACKGROUND`/`PREVIOUS` animation canvases. They do not establish the full filter × pass × depth × color-type product, every private CgBI variant, or every APNG ordering, blend, disposal, depth, and alpha combination. CRC validation is not an uncovered promised behavior: the implementation deliberately does not validate stored CRCs, as documented above.

...
Coverage audit note: direct owners now fix representative typed samples for RGB16, RGB32, CMYK16, and Lab32 compression paths; complete RGB buffers for builtin ICC and one RGB8 layer reconstruction; exact RGB/mask and white-background alpha behavior; and selected PSD/PSB structural failures including one 64-bit length extreme. The mode × depth × compression × PSD/PSB matrix remains incomplete, and masks, fills, text, vector data, blend modes, and effects are not exhaustively covered.

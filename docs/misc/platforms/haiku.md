...
CI first tries `pkgman install` against the VM's cached repository metadata. After a failed install it attempts `pkgman refresh`, tolerates refresh failure, and retries the install, for at most three install attempts. This avoids making an otherwise usable image depend on every mirror checksum before installation while still recovering from stale metadata. The VM workspace is rooted under `/boot/home`; artifact collection must preserve that prefix instead of assuming the guest path matches other BSD-family VMs.
...
| HAIKU-02 | Haiku CI retains exact Autotools and Meson matrix tuples, serial complete sixteen-part Meson slicing, cached-metadata-first installation with refresh after failure and at most three install attempts, both explicit build-OS signal skips, and all five narrow PSD skip controls. | [tests/_static/sh/staticcheck-platform-compat.sh](../../../tests/_static/sh/staticcheck-platform-compat.sh), [tests/_static/sh/staticcheck-platform-ci-contracts.sh](../../../tests/_static/sh/staticcheck-platform-ci-contracts.sh) |

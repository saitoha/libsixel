...
The same rule applies to palette tests. A shell `printf`, `dd`, pipeline, or redirection can create an RMS record file whose logical contents differ from the byte stream intended by an ACT, RIFF PAL, CR-only, CRLF, or BOM test. Byte-sensitive palette inputs and expected outputs are therefore tracked fixtures under [`tests/data/inputs/mapfile/`](../../../tests/data/inputs/mapfile/) and are marked `-text` in [`.gitattributes`](../../../.gitattributes). [`tests/_static/data/openvms-record-fixtures.tsv`](../../../tests/_static/data/openvms-record-fixtures.tsv) locks their byte counts and POSIX `cksum` values. Tests that inspect fields in a generated binary file use `stat` for the OpenVMS physical size and hexadecimal `od` output for field bytes instead of materializing another binary file through the shell. A byte-exact file produced through stdout redirection is likewise compared through normalized hexadecimal `od` output rather than `cmp`, whose OpenVMS implementation can treat different RMS record attributes as a mismatch even when the logical byte sequence is identical.

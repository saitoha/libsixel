#!/bin/sh
# Rebuild and reproduce the final palette merge comparison artifacts.
...
warmups=${MERGE_POLICY_WARMUPS-2}
runs=${MERGE_POLICY_RUNS-7}
source_state=clean
...
"${PYTHON}" "${TOP_SRCDIR}/tools/plot_merge_policy_measurements.py" \
    "${input_image}" \
    ...
    --warmups "${warmups}" \
    --runs "${runs}" \
    --output-csv "${output_dir}/merge-policy-comparison.csv" \
    --output-quality-plot "${output_dir}/merge-policy-quality.png" \
    --output-performance-plot \
    "${output_dir}/merge-policy-performance.png" \
    --output-metadata "${output_dir}/merge-policy-run.json"

"${PYTHON}" "${TOP_SRCDIR}/tools/check_merge_policy_measurements.py" \
    "${output_dir}"

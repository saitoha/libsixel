...
def measure(args):
    ...
    metadata = {"schema": 1, "revision": run(["git", "-C", build, "rev-parse", "HEAD"]).decode().strip(),
                ...
                "timing_boundaries": {
                    ...
                    "uncertainty": f"median and IQR of {args.runs} samples after two warmups; no CPU affinity or exclusive host"},
...
def verify(output):
    ...
    assert len(metadata["rows"]) == len(metadata["fixtures"]) * len(MODES)
    assert {(r["fixture"], r["mode"]) for r in metadata["rows"]} == {
        (name, mode) for name in metadata["fixtures"] for mode in MODES}
    assert metadata["timings"], "timing collection is incomplete"
...
def render(output):
    ...
    for name in ("snake", "autumn", "gradient", "gray", "revisit"):
        ...
        for axis in axes.flat:
            axis.axis("off")
        fig.colorbar(plotted, ax=list(axes[1]), shrink=.8, label="Mean |RGB8 error|")
    ...
    with (output / "timings.csv").open("w") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(metadata["timings"][0]), lineterminator="\n")
        writer.writeheader()

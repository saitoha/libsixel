#!/bin/sh
# TAP test: quicklook decodes ZIP TIFF with stable visual quality.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" --env SIXEL_THUMBNAILER_HINT_SIZE=64 -L quicklook! \
    ...
    >"${ARTIFACT_LOCAL_DIR}/quicklook_tiff_zip_rgb.six" || {
    echo "not ok" 1 - "quicklook TIFF zip rgb decode failed"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:0.98" \
    ...
    "${ARTIFACT_LOCAL_DIR}/quicklook_tiff_zip_rgb.six" 2>&1) || {
    echo "not ok" 1 - "$lsqa_msg"
    exit 0
}

echo "ok" 1 - "quicklook TIFF zip rgb decode preserves quality"
exit 0

#!/bin/sh
# TAP test: quicklook decodes RGB TGA with stable visual quality.
...
run_img2sixel --env SIXEL_THUMBNAILER_HINT_SIZE=64 -L quicklook! \
    ...
    >"${ARTIFACT_LOCAL_DIR}/quicklook_tga_rgb.six" || {
    echo "not ok" 1 - "quicklook TGA rgb decode failed"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" \
    "${TOP_SRCDIR}/tests/data/inputs/formats/snake-64-reference-rgb.ppm" \
    "${ARTIFACT_LOCAL_DIR}/quicklook_tga_rgb.six" 2>&1) || {
    echo "not ok" 1 - "$lsqa_msg"
    exit 0
}

echo "ok" 1 - "quicklook TGA rgb decode preserves quality"
exit 0

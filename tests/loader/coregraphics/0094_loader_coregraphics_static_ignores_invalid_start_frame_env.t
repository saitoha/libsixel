#!/bin/sh
# TAP test: coregraphics static decode ignores invalid start-frame env values.
...
error_text="$(${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    echo "not ok 1 - coregraphics static decode failed with invalid env"
    exit 0
}

test "${error_text#*SIXEL_LOADER_ANIMATION_START_FRAME_NO*}" = \
    "${error_text}" || {
    echo "not ok 1 - coregraphics static decode emitted start-frame env error text"
    exit 0
}

echo "ok 1 - coregraphics static decode ignores invalid start-frame env values"
exit 0

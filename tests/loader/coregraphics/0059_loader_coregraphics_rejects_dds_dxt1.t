#!/bin/sh
# TAP test: CoreGraphics decodes DDS DXT1 input when macOS supports DDS UTI.
...
output_sixel="${ARTIFACT_LOCAL_DIR}/coregraphics_dds_dxt1.six"

run_img2sixel -L coregraphics! "${image_path}" >"${output_sixel}" || {
    echo "not ok" 1 "coregraphics failed to decode DDS DXT1 input"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 "coregraphics decodes DDS DXT1 with expected quality"
    exit 0
}
...
echo "not ok" 1 "coregraphics DDS DXT1 quality regressed"
exit 0

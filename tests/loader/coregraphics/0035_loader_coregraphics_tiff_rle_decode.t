#!/bin/sh
# TAP test: coregraphics loader keeps MS-SSIM baseline for RLE TIFF input.
...
output_sixel="${ARTIFACT_LOCAL_DIR}/coregraphics_tiff_rle.six"

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L coregraphics! "${image_path}" >"${output_sixel}" || {
    echo "not ok" 1 - "coregraphics failed to decode RLE TIFF input"
    exit 0
}
...
echo "ok" 1 - "coregraphics keeps MS-SSIM baseline for RLE TIFF input"
exit 0

#!/bin/sh
# Verify libjpeg loader applies embedded JPEG ICC profile when cms=1.
...
output_sixel="${ARTIFACT_LOCAL_DIR}/libjpeg_jpeg_embedded_esrgb_cms1.six"

run_img2sixel -L libjpeg:cms=1! "${input_jpeg}" >"${output_sixel}" || {
    echo "not ok" 1 - "libjpeg embedded-ICC JPEG decode failed (cms=1)"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_path}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "libjpeg embedded-ICC JPEG (cms=1) fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "libjpeg embedded-ICC JPEG with cms=1 keeps MS-SSIM ${lsqa_floor} against expected PNM"
exit 0

#!/bin/sh
# Verify libjpeg loader decodes 8-bit CMYK JPEG (sequential 4:4:4)
...
output_sixel="${ARTIFACT_LOCAL_DIR}/libjpeg_jpeg_cmyk_8bit_seq444_r0.six"

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L libjpeg! "${input_jpeg}" >"${output_sixel}" || {
    echo "not ok" 1 - "libjpeg CMYK 8-bit sequential 4:4:4 JPEG decode failed"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_path}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "libjpeg CMYK 8-bit sequential 4:4:4 JPEG fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "libjpeg CMYK 8-bit sequential 4:4:4 JPEG keeps MS-SSIM ${lsqa_floor} against expected PNM"
exit 0

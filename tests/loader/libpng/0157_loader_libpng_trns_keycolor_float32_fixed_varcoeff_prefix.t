#!/bin/sh
# Verify float32 fs+carry keeps keycolor header for a tRNS keycolor sample.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -A composite --env SIXEL_LOADER_LIBPNG_USE_TRNS_KEYCOLOR=1 \
              --env SIXEL_THREADS=4 \
              -Llibpng:cms_engine=none! \
              -d fs:scan=raster --precision=float32 \
...
case "${output_six_text}" in
    *"$(printf '\033')P0;1q"*)
        echo "ok 1 - float32 fs carry keeps keycolor header"
        ;;
    *)
        echo "not ok 1 - float32 fs carry lost keycolor header"
        ;;
esac

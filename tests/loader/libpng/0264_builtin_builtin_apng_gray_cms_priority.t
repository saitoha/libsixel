...
${SIXEL_RUNTIME-} "${TEST_RUNNER_PATH}" loader/libpng_contract \
    priority builtin builtin apng gray || result=$?
...
test "$result" -eq 0 || {
    echo "not ok 1 - builtin builtin APNG gray metadata priority"
    exit 0
}
echo "ok 1 - builtin builtin APNG gray metadata priority"
exit 0

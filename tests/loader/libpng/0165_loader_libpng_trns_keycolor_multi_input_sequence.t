...
out_on="${TMPDIR:-/tmp}/libsixel-${0##*/}-$$-trns-keycolor-multi-on-ab.six"
out_off="${TMPDIR:-/tmp}/libsixel-${0##*/}-$$-trns-keycolor-multi-off-ab.six"
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -Akeep --env SIXEL_LOADER_LIBPNG_USE_TRNS_KEYCOLOR=0 \
              --env SIXEL_THREADS=4 \
              -Llibpng:cms_engine=none! -d fs:scan=raster \
...
test "${out_on_has_header}" = 1 || {
    echo "not ok 1 - multi-input keycolor mask preservation mismatch"
    exit 0
}

test "${out_off_has_header}" = 1 || {
    echo "not ok 1 - multi-input keycolor mask preservation mismatch"
    exit 0
}

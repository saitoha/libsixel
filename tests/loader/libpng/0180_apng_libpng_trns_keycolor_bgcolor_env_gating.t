#!/bin/sh
# Verify process SIXEL_BGCOLOR disables APNG keycolor path.
...
out_on="${ARTIFACT_LOCAL_DIR}/apng-trns-keycolor-bg-proc-on.six"
out_off="${ARTIFACT_LOCAL_DIR}/apng-trns-keycolor-bg-proc-off.six"
...
cmp -s "${out_on}" "${out_off}" || {
    echo "not ok 1 - process SIXEL_BGCOLOR unexpectedly keeps APNG keycolor path"
    exit 0
}

    echo "ok 1 - process SIXEL_BGCOLOR disables APNG keycolor path"

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    echo "not ok" 1 - "wic ico env legacy-name conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "wic ico legacy env name quality meets baseline"
    exit 0
}
...
echo "not ok" 1 - "wic ico legacy env name quality regressed"
exit 0

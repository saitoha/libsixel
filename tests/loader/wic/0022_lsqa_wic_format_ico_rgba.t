...
#   convert tests/data/inputs/formats/snake-64-reference-rgba.ppm tests/data/inputs/formats/snake-ico-rgba.ico
...
run_img2sixel -Lwic! "${image_path}" >"${output_sixel}" || {
    echo "not ok" 1 - "wic ico rgba conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "wic ico rgba quality meets baseline"
    exit 0
}
...
echo "not ok" 1 - "wic ico rgba quality regressed"

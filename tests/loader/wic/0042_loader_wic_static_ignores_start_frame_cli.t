#!/bin/sh
# TAP test: wic static decode ignores --start-frame overrides.
...
error_text="$(${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    echo "not ok 1 - wic static decode failed with --start-frame=999"
    exit 0
}

test "${error_text#*SIXEL_LOADER_ANIMATION_START_FRAME_NO*}" = \
    "${error_text}" || {
    echo "not ok 1 - wic static decode emitted --start-frame error text"
    exit 0
}

echo "ok 1 - wic static decode ignores --start-frame overrides"
exit 0

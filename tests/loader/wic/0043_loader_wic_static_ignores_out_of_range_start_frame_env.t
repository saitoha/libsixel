#!/bin/sh
# TAP test: wic static decode ignores out-of-range start-frame env values.
...
error_text="$(${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    echo "not ok 1 - wic static decode failed with out-of-range env"
    exit 0
}

test "${error_text#*SIXEL_LOADER_ANIMATION_START_FRAME_NO*}" = \
    "${error_text}" || {
    echo "not ok 1 - wic static decode emitted out-of-range env error text"
    exit 0
}

echo "ok 1 - wic static decode ignores out-of-range start-frame env values"
exit 0

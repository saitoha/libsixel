#!/bin/sh
# TAP test confirming stdin .svgz tempfile close failure reports diagnostics.
...
test "${status}" -ne 0 || {
    echo "not ok" 1 - "stdin .svgz tempfile close failpoint unexpectedly succeeded"
    exit 0
}
...
case "${msg}" in
    *"failed to close temporary .svgz file."*)
        ;;
    *)
        echo "not ok" 1 - "stdin .svgz tempfile close failure diagnostics missing"
        exit 0
        ;;
esac

echo "ok" 1 - "stdin .svgz tempfile close failure diagnostics are reported"
exit 0

#!/bin/sh
# Verify PSB mode7 CMYK16 non-pixel valid ICC matrix avoids false embedded-ICC failure traces.
...
for suffix in \
 ...
 do
    input_psd="${base_dir}/snake16_psb_mode7_cmyk16_missing_composite_multilayer_${suffix}_valid_icc_profile.psd"
    command_status=0
    ...
    case "${command_status}" in
        0)
            ;;
        *)
            echo "not ok" 1 - "PSB mode7 CMYK16 non-pixel valid ICC matrix decode failed for ${suffix}: ${trace_log}"
            status=1
            break
            ;;
    esac
    ...
    case "${trace_log}" in
        *"embedded ICC conversion failed"*)
            echo "not ok" 1 - "PSB mode7 CMYK16 non-pixel valid ICC matrix emitted false failure trace for ${suffix}"
            status=1
            break
            ;;
    esac
...
echo "ok" 1 - "PSB mode7 CMYK16 non-pixel valid ICC matrix avoids false embedded-ICC failure traces"
exit 0

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L builtin! -o "${output_builtin}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "builtin palette_lossless_noicc_anim2.webp second-frame decode failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L libwebp! -o "${output_libwebp}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp palette_lossless_noicc_anim2.webp second-frame decode failed"
    exit 0
}
...
echo "ok" 1 - "builtin palette_lossless_noicc_anim2.webp second-frame quality keeps MS-SSIM >= 0.98"
exit 0

#!/bin/sh
# Verify clbl=1 deferred solid overlay keeps destination alpha unchanged.
...
test "${trace_output#*builtin PSD: applying clip-weighted deferred solid overlay in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "blend_and_clipping missing deferred solid overlay trace"
    exit 0
}
...
test "${trace_output#*builtin PSD: keeping deferred solid overlay alpha unchanged in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "blend_and_clipping missing deferred solid alpha-invariant trace"
    exit 0
}

echo "ok" 1 - \
    "blend_and_clipping keeps deferred solid overlay alpha invariant"
exit 0

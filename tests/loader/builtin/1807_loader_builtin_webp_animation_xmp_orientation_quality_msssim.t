#!/bin/sh
# TAP test confirming animated XMP fixture first-frame quality with orientation off.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L builtin! -ldisable \
    ...
    echo "not ok" 1 - "builtin animated XMP fixture quality decode failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L libwebp! -ldisable \
    -o "${output_libwebp}" "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp animated XMP fixture quality decode failed"
    exit 0
}
...
echo "ok" 1 - "builtin animated XMP fixture quality keeps MS-SSIM >= 0.98"
exit 0

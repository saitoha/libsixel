#!/bin/sh
# Verify mode7(4ch->CMYK16) TySh descriptor payload on a pixel layer keeps
# pixel-priority semantics: non-pixel payload is ignored without synthetic fill
# and output remains the CMYK16 snake baseline.
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 CMYK16 TySh descriptor non-pixel/pixel-layer PSD: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    *"builtin PSD: ignoring non-pixel payload in layer fallback"*)
        ;;
    *)
        echo "not ok" 1 - "mode7 CMYK16 TySh descriptor non-pixel/pixel-layer ignore info trace is missing"
        exit 0
        ;;
esac
...
case "${trace_output}" in
    *"builtin PSD: rendering non-pixel fill payload in layer fallback"*)
        echo "not ok" 1 - "mode7 CMYK16 TySh descriptor non-pixel/pixel-layer unexpectedly rendered synthetic fill"
        exit 0
        ;;
    *)
        ;;
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "mode7 CMYK16 TySh descriptor non-pixel/pixel-layer decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "mode7 CMYK16 TySh descriptor non-pixel/pixel-layer keeps pixel-priority and deterministic trace with MS-SSIM ${lsqa_floor}"
exit 0

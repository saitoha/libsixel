#!/bin/sh
# Verify PSD CMYK8 TySh EngineData FillFlag=false + StrokeFlag=true renders stroke payload.
...
input_psd="${base_dir}/snake16_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillflag_false_strokeflag_true.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode CMYK8 TySh StrokeFlag=true fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "CMYK8 TySh StrokeFlag=true decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD CMYK8 TySh StrokeFlag=true keeps stroke trace and MS-SSIM ${lsqa_floor}"
exit 0

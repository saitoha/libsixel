...
test "${status_bg}" -eq 0 || {
    echo "not ok 1 - missing-composite single-layer Duotone8+alpha render failed with --bgcolor"
    exit 0
}

test "${output_bg#*"${keycolor_header}"}" = "${output_bg}" || {
    echo "not ok 1 - missing-composite single-layer Duotone8+alpha kept keycolor header with --bgcolor"
    exit 0
}

echo "ok 1 - missing-composite single-layer Duotone8+alpha suppresses keycolor header with --bgcolor"
exit 0

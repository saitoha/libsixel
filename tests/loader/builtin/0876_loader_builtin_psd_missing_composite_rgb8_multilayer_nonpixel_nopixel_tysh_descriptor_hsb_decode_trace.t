#!/bin/sh
# Verify RGB8 TySh descriptor HSB payload in a non-pixel/no-pixel
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode RGB8 TySh descriptor HSB non-pixel/no-pixel PSD: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    *"builtin PSD: ignoring non-pixel payload in layer fallback"*)
        ;;
    *)
        echo "not ok" 1 - "RGB8 TySh descriptor HSB non-pixel/no-pixel ignore info trace is missing"
        exit 0
        ;;
esac
...
case "${trace_output}" in
    *"builtin PSD: rendering non-pixel fill payload in layer fallback"*)
        ;;
    *)
        echo "not ok" 1 - "RGB8 TySh descriptor HSB non-pixel/no-pixel fill render trace is missing"
        exit 0
        ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "RGB8 TySh descriptor HSB non-pixel/no-pixel decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "RGB8 TySh descriptor HSB non-pixel/no-pixel layer renders synthetic fill with deterministic trace and keeps MS-SSIM ${lsqa_floor}"
exit 0

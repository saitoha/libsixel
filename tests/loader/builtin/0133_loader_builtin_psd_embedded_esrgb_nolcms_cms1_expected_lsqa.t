...
output_sixel="${ARTIFACT_LOCAL_DIR}/builtin_psd_embedded_esrgb_nolcms_cms1.six"

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -Lbuiltin:cms_engine=auto! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "builtin embedded-ICC PSD decode failed (nolcms, cms=1)"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_path}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "builtin embedded-ICC PSD (nolcms, cms=1) fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "builtin embedded-ICC PSD (nolcms, cms=1) keeps MS-SSIM ${lsqa_floor} against expected PNM"
exit 0

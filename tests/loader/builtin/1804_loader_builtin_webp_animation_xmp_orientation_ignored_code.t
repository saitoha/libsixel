...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin animated XMP orientation disabled decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin animated XMP orientation disabled missing LSXWEBP1 header"
    exit 0
}
...
test "${diag_line#LSXWEBP1\|rc=0\|kind=OK\|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP orientation disabled malformed success header"
    exit 0
}

test "${diag_line#*W_OK_ANIM*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP orientation disabled missing W_OK_ANIM"
    exit 0
}

test "${diag_line#*W_META_XMP_IGNORED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP orientation disabled missing W_META_XMP_IGNORED"
    exit 0
}

test "${diag_line#*W_META_XMP_APPLIED*}" = "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP orientation disabled unexpectedly emitted W_META_XMP_APPLIED"
    exit 0
}

echo "ok" 1 - "builtin animated XMP orientation disabled emits IGNORED contract"
exit 0

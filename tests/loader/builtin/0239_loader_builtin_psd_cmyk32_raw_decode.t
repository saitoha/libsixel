#!/bin/sh
# Verify builtin loader decodes PSD CMYK 32-bit Raw with stable image quality.
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_cmyk32_raw.psd"
reference_ppm="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_cmyk32_expected.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_cmyk32_raw_output.six"
lsqa_floor=${LSQA_MS_SSIM_FLOOR:-0.99}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "PSD CMYK 32-bit Raw decode failed"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -P float32 -b "MS-SSIM:${lsqa_floor}" \
    "${reference_ppm}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "PSD CMYK 32-bit Raw fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD CMYK 32-bit Raw keeps MS-SSIM ${lsqa_floor} against expected PPM"
exit 0

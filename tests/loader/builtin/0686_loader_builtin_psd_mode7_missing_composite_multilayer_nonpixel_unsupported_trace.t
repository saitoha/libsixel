#!/bin/sh
# Verify mode7 non-pixel layer payload in multi-layer fallback is rejected with
...
case "${trace_output}" in
    *"builtin PSD: unsupported non-pixel layer in layer fallback"*)
        echo "ok" 1 - "mode7 non-pixel multi-layer payload is rejected as unsupported"
        ;;
    *)
        echo "not ok" 1 - "mode7 non-pixel multi-layer unsupported trace is missing"
        ;;
esac

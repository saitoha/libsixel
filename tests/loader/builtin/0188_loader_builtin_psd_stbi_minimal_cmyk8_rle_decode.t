#!/bin/sh
# Verify builtin loader decodes CMYK 8-bit RLE with stable image quality.
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/stbi_minimal_cmyk8_rle.psd"
reference_six="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_cmyk8_raw_builtin_expected.six"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_cmyk8_rle_output.six"
lsqa_floor=${LSQA_MS_SSIM_FLOOR_PSD_CMYK8:-0.995}

run_img2sixel -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "builtin loader failed to decode CMYK 8-bit RLE"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_six}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "builtin CMYK 8-bit RLE fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "builtin CMYK 8-bit RLE keeps MS-SSIM ${lsqa_floor} against expected sixel"
exit 0

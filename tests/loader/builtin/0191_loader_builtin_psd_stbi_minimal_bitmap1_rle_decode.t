#!/bin/sh
# Verify builtin loader decodes bitmap 1-bit RLE with stable image quality.
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_bitmap1_rle.psd"
reference_ppm="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_bitmap1_expected.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_bitmap1_rle_output.six"
...
lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "builtin bitmap 1-bit RLE fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "builtin bitmap 1-bit RLE keeps MS-SSIM ${lsqa_floor} against expected PPM"
exit 0

#!/bin/sh
# TAP test confirming animated WebP subrect blend/dispose keeps quality parity.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    echo "not ok" 1 - "builtin animated WebP subrect blend/dispose decode failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    -L libwebp! -ldisable -o "${output_libwebp}" "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp animated WebP subrect blend/dispose decode failed"
    exit 0
}
...
echo "ok" 1 - "animated WebP subrect blend/dispose keeps MS-SSIM >= 0.98"
exit 0

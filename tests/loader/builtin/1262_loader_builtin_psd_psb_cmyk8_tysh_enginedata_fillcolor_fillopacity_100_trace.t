#!/bin/sh
# Verify PSB CMYK8 TySh FillOpacity=1.0 keeps full synthetic fill render.
...
input_psd="${base_dir}/snake16_psb_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillopacity_100.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode PSB CMYK8 TySh FillOpacity=1.0 fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "PSB CMYK8 TySh FillOpacity=1.0 decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSB CMYK8 TySh FillOpacity=1.0 keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

#!/bin/sh
# Verify builtin loader reconstructs mode7(3ch->RGB32) multi-layer PSD
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_mode7_rgb32_missing_composite_multilayer_normal.psd"
reference_ppm="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_multilayer_rgb32_normal_expected.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_mode7_missing_composite_multilayer_rgb32_normal_output.six"
lsqa_floor=${LSQA_MS_SSIM_FLOOR:-0.995}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 multi-layer PSD RGB32(normal)"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_ppm}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "mode7 multi-layer PSD RGB32(normal) fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "mode7 multi-layer PSD RGB32(normal) keeps MS-SSIM ${lsqa_floor} against expected PPM"
exit 0

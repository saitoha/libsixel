#!/bin/sh
# Verify FXPRI_INSIDE overlap keeps quality with unit-alpha candidates.
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - \
        "effects/stroke-composite FXPRI unit-alpha LSQA fell below ${lsqa_floor}"
    exit 0
}

: "${lsqa_msg}"

echo "ok" 1 - "effects/stroke-composite keeps FXPRI unit-alpha quality"
exit 0

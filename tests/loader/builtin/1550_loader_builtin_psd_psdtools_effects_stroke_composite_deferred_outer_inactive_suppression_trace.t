...
test "${trace_output#*builtin PSD: applying clip-weighted deferred outer effects in layer fallback*}" \
    = "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite unexpectedly entered deferred outer apply path"
    exit 0
}
...
test "${trace_output#*builtin PSD: applying deferred outer alpha accumulation in layer fallback*}" \
    = "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite unexpectedly emitted deferred outer alpha accumulation trace"
    exit 0
}
...
test "${trace_output#*builtin PSD: clbl=1; deferring interior overlays to clipped group composite*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite lost clbl=1 deferred contract"
    exit 0
}
...
test "${trace_output#*builtin PSD: applying clip-weighted deferred gradient overlay in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite lost deferred gradient overlay under inactive outer suppression"
    exit 0
}
...
test "${trace_output#*builtin PSD: applying clip-weighted deferred effect stroke in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite lost deferred stroke under inactive outer suppression"
    exit 0
}

echo "ok" 1 - \
    "effects/stroke-composite keeps deferred flow while inactive outer apply stays suppressed"
exit 0

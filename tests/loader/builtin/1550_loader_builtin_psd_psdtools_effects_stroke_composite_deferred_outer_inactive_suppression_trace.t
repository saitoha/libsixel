...
case "${diag_line}" in
    *FX_DEFERRED_OUTER_CLIP*)
        echo "not ok" 1 - "effects/stroke-composite unexpectedly emitted FX_DEFERRED_OUTER_CLIP"
        exit 0
        ;;
esac

case "${diag_line}" in
    *FX_DEFERRED_OUTER_ALPHA_ACCUM*)
        echo "not ok" 1 - "effects/stroke-composite unexpectedly emitted FX_DEFERRED_OUTER_ALPHA_ACCUM"
        exit 0
        ;;
esac

echo "ok" 1 - \
    "effects/stroke-composite keeps inactive outer suppression diagnostic contract"
exit 0

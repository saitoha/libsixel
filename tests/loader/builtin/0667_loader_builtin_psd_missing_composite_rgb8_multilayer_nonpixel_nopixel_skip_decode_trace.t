...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_rgb8_missing_composite_multilayer_nonpixel_nopixel_tysh.psd"
reference_ppm="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_rgb8_expected.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_missing_composite_multilayer_nonpixel_nopixel_output.six"
...
case "${trace_output}" in
    *"builtin PSD: ignoring non-pixel payload in layer fallback"*)
        ;;
    *)
        echo "not ok" 1 - "non-pixel/no-pixel ignore info trace is missing"
        exit 0
        ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "non-pixel/no-pixel multi-layer decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "non-pixel/no-pixel layer is skipped with info trace and decode keeps MS-SSIM ${lsqa_floor}"
exit 0

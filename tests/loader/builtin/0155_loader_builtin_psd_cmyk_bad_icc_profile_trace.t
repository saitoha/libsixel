#!/bin/sh
# Verify builtin PSD CMYK decoder logs ICC conversion failure for invalid profile bytes.
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/stbi_minimal_cmyk8_bad_icc_profile.psd"
...
case "${trace_log}" in
    *"libsixel: builtin PSD: embedded ICC conversion failed"*)
        ;;
    *)
        echo "not ok" 1 - "missing PSD CMYK embedded-ICC conversion failure trace"
        exit 0
        ;;
esac
...
echo "ok" 1 - "builtin PSD CMYK logs ICC conversion failure for invalid profile bytes"
exit 0

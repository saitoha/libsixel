#!/bin/sh
# Verify malformed PSD TySh EngineData /Color /Values [/DeviceRGB] payload is
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_color_values_named_device_rgb_malformed_payload.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode malformed PSD TySh /Color /DeviceRGB payload fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "malformed PSD TySh /Color /DeviceRGB payload decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "malformed PSD TySh /Color /DeviceRGB payload is skipped with deterministic trace and keeps MS-SSIM ${lsqa_floor}"
exit 0

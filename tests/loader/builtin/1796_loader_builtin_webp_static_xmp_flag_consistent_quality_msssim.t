#!/bin/sh
# TAP test confirming static XMP-consistent WebP quality against libwebp.
...
SIXEL_LOADER_ORIENTATION=off
export SIXEL_LOADER_ORIENTATION

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L builtin! -o "${output_builtin}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "builtin static XMP fixture decode failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L libwebp! -o "${output_libwebp}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp static XMP fixture decode failed"
    exit 0
}

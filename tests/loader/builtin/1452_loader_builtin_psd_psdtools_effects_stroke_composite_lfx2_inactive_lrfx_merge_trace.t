...
test "${trace_output#*builtin PSD: ignoring legacy lrFX when lfx2 is present*}" \
    = "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite still ignored legacy lrFX effects"
    exit 0
}

echo "ok" 1 - "effects/stroke-composite merges legacy lrFX for inactive lfx2 effects"
exit 0

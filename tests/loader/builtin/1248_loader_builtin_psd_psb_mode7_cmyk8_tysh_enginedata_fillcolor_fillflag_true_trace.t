#!/bin/sh
# Verify PSB mode7(4ch->CMYK8) TySh EngineData FillFlag=true keeps synthetic fill render.
...
input_psd="${base_dir}/snake16_psb_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillflag_true.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode PSB mode7 CMYK8 TySh FillFlag=true fixture: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    *"builtin PSD: skipping non-pixel fill payload due to FillFlag=false"*)
        echo "not ok" 1 - "FillFlag=false skip trace must not appear when FillFlag=true"
        exit 0
        ;;
    *)
        ;;
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "PSB mode7 CMYK8 TySh FillFlag=true decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSB mode7 CMYK8 TySh FillFlag=true keeps render trace and MS-SSIM ${lsqa_floor}"
exit 0

#!/bin/sh
# Verify IrSh offset semantics are parsed and deferred interior path stays active.
...
test "${trace_output#*builtin PSD: parsed IrSh shadow offset semantics in layer effects*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite did not parse IrSh offset semantics"
    exit 0
}
...
test "${trace_output#*builtin PSD: applying clip-weighted deferred interior effects in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite lost deferred interior effect contract"
    exit 0
}

echo "ok" 1 - "effects/stroke-composite keeps IrSh shadow-offset semantics contract"
exit 0

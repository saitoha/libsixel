#!/bin/sh
# Verify PSB mode7 CMYK8 non-pixel valid ICC matrix avoids false embedded-ICC failure traces.
...
for suffix in \
 ...
 do
    input_psd="${base_dir}/snake16_psb_mode7_cmyk8_missing_composite_multilayer_${suffix}_valid_icc_profile.psd"
    command_status=0
    trace_log=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -v -Lbuiltin:cms=auto! \
        "${input_psd}" -o /dev/null 2>&1) || command_status=$?

    test "${command_status}" -eq 0 || {
        echo "not ok" 1 - "PSB mode7 CMYK8 non-pixel valid ICC matrix decode failed for ${suffix}: ${trace_log}"
        status=1
        break
    }
    ...
    case "${trace_log}" in
        *"embedded ICC conversion failed"*)
            echo "not ok" 1 - "PSB mode7 CMYK8 non-pixel valid ICC matrix emitted false failure trace for ${suffix}"
            status=1
            break
            ;;
    esac
...
echo "ok" 1 - "PSB mode7 CMYK8 non-pixel valid ICC matrix avoids false embedded-ICC failure traces"
exit 0

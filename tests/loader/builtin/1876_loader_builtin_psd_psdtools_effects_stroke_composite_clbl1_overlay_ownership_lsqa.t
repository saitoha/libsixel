#!/bin/sh
# Verify clbl=1 deferred overlay ownership keeps stroke-composite quality.
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - \
        "effects/stroke-composite clbl1 overlay ownership LSQA fell below ${lsqa_floor}"
    exit 0
}

: "${lsqa_msg}"

echo "ok" 1 - "effects/stroke-composite keeps clbl1 overlay ownership quality"
exit 0

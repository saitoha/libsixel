#!/bin/sh
# Verify deferred outer distance-band path keeps interior/stroke contracts.
...
test "${trace_output#*builtin PSD: suppressing clbl=1 deferred base interior glow/choke/bevel-shadow*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite lost deferred interior suppression"
    exit 0
}
...
echo "ok" 1 - "effects/stroke-composite keeps deferred outer non-regression contract"
exit 0

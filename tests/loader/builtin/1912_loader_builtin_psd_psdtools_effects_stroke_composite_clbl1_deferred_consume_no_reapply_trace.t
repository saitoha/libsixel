...
test "${committed_tail#*builtin PSD: replaying deferred clbl=1 overlay entry in layer fallback*}" \
    = "${committed_tail}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite replayed deferred overlay after commit consume"
    exit 0
}
...
test "${committed_tail#*builtin PSD: applying clip-weighted deferred gradient overlay in layer fallback*}" \
    = "${committed_tail}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite applied deferred gradient overlay after commit consume"
    exit 0
}
...
test "${committed_tail#*builtin PSD: applying deferred stroke on clipped group*}" \
    = "${committed_tail}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite applied deferred stroke after commit consume"
    exit 0
}

test "${committed_tail#*builtin PSD: applying deferred vector stroke and layer effect stroke on clipped group*}" \
    = "${committed_tail}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite applied deferred dual-stroke after commit consume"
    exit 0
}
...
test "${committed_tail#*builtin PSD: applying clip-weighted deferred solid overlay in layer fallback*}" \
    = "${committed_tail}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite applied deferred solid overlay after commit consume"
    exit 0
}

echo "ok" 1 - \
    "effects/stroke-composite keeps deferred apply traces out of post-commit tail"
exit 0

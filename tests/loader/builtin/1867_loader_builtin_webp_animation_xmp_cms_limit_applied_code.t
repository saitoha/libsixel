...
input_webp="${TOP_SRCDIR}/tests/data/inputs/formats/animated-lossy-8x8-2frame-min-xmp-icc-srgb-limit.webp"
trace_output=''
diag_line=''
command_status=0
nl='
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin animated XMP CMS limit decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin animated XMP CMS limit missing LSXWEBP1 header"
    exit 0
}
...
test "${diag_line#LSXWEBP1\|rc=0\|kind=OK\|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS limit malformed success header"
    exit 0
}

test "${diag_line#*W_OK_ANIM*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS limit missing W_OK_ANIM"
    exit 0
}

test "${diag_line#*W_META_XMP_CMS_APPLIED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS limit missing W_META_XMP_CMS_APPLIED"
    exit 0
}

test "${diag_line#*W_META_XMP_CMS_SIZE_LIMIT_IGNORED*}" = "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS limit unexpectedly emitted W_META_XMP_CMS_SIZE_LIMIT_IGNORED"
    exit 0
}

echo "ok" 1 - "builtin animated XMP CMS at parse limit stays applied"
exit 0

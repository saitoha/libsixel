...
test "${trace_output#*builtin PSD: applying clip-weighted deferred interior effects in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite did not enter deferred interior effect path"
    exit 0
}
...
echo "ok" 1 - "effects/stroke-composite deferred interior effects use distance-map coverage"
exit 0

...
# Compare composited RGB output to avoid loader-specific alpha channel storage.
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    -B "#112233" -L builtin! -ldisable -o "${output_builtin}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "builtin animated WebP lossy alpha decode failed"
    exit 0
}
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp animated WebP lossy alpha decode failed"
    exit 0
}
...
echo "ok" 1 - "animated WebP lossy alpha composited quality keeps MS-SSIM >= 0.98"
exit 0

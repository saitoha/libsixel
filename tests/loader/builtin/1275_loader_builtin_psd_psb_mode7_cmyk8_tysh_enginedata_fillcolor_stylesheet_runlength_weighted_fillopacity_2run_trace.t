#!/bin/sh
# Verify PSB mode7(4ch->CMYK8) TySh StyleRun weighted FillOpacity keeps run-length weighted opacity.
...
input_psd="${base_dir}/snake16_psb_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_stylesheet_runlength_weighted_fillopacity_2run.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode PSB mode7 CMYK8 TySh weighted FillOpacity fixture: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    *"builtin PSD: rendering non-pixel stroke payload in layer fallback"*)
        echo "not ok" 1 - "stroke payload render trace must not appear in weighted FillOpacity path"
        exit 0
        ;;
    *) ;;
esac
...
case "${trace_output}" in
    *"builtin PSD: malformed TySh FillOpacity; ignoring"*)
        echo "not ok" 1 - "malformed FillOpacity trace must not appear for valid weighted FillOpacity"
        exit 0
        ;;
    *) ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "PSB mode7 CMYK8 TySh weighted FillOpacity decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSB mode7 CMYK8 TySh weighted FillOpacity keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

#!/bin/sh
# Verify mode7 unknown blend key in multi-layer fallback is rejected with
...
case "${trace_output}" in
    *"builtin PSD: unsupported layer blend mode"*)
        echo "ok" 1 - "mode7 unknown multi-layer blend mode is rejected as unsupported"
        ;;
    *)
        echo "not ok" 1 - "mode7 unknown multi-layer blend unsupported trace is missing"
        ;;
esac

#!/bin/sh
# Verify builtin loader decodes PSD Lab 32-bit ZIP with stable image quality.
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_lab32_zip.psd"
reference_six="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_lab32_coregraphics_expected.six"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_lab32_zip_output.six"
lsqa_floor=${LSQA_MS_SSIM_FLOOR:-0.99}

run_img2sixel -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "PSD Lab 32-bit ZIP decode failed"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_six}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "PSD Lab 32-bit ZIP fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD Lab 32-bit ZIP keeps MS-SSIM ${lsqa_floor} against expected sixel"
exit 0

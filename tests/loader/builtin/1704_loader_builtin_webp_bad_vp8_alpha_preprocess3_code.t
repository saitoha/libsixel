...
input_webp="${TOP_SRCDIR}/tests/data/corrupted/bad_vp8_alpha_preprocess3.webp"
trace_output=''
diag_line=''
command_status=0
nl='
...
test "${command_status}" -ne 0 || {
    echo "not ok" 1 - "forced builtin loader VP8+ALPHA preprocess=3 fixture unexpectedly succeeded"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "forced builtin loader VP8+ALPHA preprocess=3 fixture missing LSXWEBP1 contract header"
    exit 0
}
...
test "${diag_line#LSXWEBP1\|rc=1\|kind=ERR\|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "forced builtin loader VP8+ALPHA preprocess=3 fixture malformed error contract header"
    exit 0
}

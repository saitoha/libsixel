#!/bin/sh
# Verify FXPRI_INSIDE overlap keeps quality with shared-overlap normalization.
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - \
        "effects/stroke-composite FXPRI shared-overlap LSQA fell below ${lsqa_floor}"
    exit 0
}

: "${lsqa_msg}"

echo "ok" 1 - "effects/stroke-composite keeps FXPRI shared-overlap quality"
exit 0

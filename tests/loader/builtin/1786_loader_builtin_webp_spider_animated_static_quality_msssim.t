#!/bin/sh
# TAP test confirming builtin spider animated first-frame quality parity.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L builtin! -o "${output_builtin}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "builtin spider animated first-frame decode failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L libwebp! -o "${output_libwebp}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp spider animated first-frame decode failed"
    exit 0
}
...
echo "ok" 1 - "builtin spider animated first-frame quality keeps MS-SSIM >= 0.98"
exit 0

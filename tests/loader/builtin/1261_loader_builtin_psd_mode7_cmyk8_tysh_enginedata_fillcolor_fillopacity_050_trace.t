#!/bin/sh
# Verify PSD mode7(4ch->CMYK8) TySh FillOpacity=0.50 applies synthetic fill opacity.
...
input_psd="${base_dir}/snake16_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillopacity_050.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 CMYK8 TySh FillOpacity=0.50 fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "mode7 CMYK8 TySh FillOpacity=0.50 decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD mode7 CMYK8 TySh FillOpacity=0.50 keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "builtin loader failed to decode PSB mode7(4ch->CMYK)32 multi-layer(normal high-offset-xxlarge)"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    "${reference_ppm}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "PSB mode7(4ch->CMYK)32 multi-layer(normal high-offset-xxlarge) fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSB mode7(4ch->CMYK)32 multi-layer(normal high-offset-xxlarge) keeps MS-SSIM ${lsqa_floor} against expected PPM"
exit 0

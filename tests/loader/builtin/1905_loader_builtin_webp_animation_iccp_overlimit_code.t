#!/bin/sh
# TAP test confirming oversize ICCP is ignored non-fatally on animated WebP.
...
input_webp="${TOP_BUILDDIR}/tests/data/inputs/formats/animated-lossy-8x8-2frame-min-embedded-srgb-icc-overlimit.webp"
trace_output=''
diag_line=''
command_status=0
nl='
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin animated ICCP overlimit decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin animated ICCP overlimit missing LSXWEBP1"
    exit 0
}
...
test "${diag_line#*W_OK_ANIM*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated ICCP overlimit missing W_OK_ANIM"
    exit 0
}

test "${diag_line#*W_META_ICCP_IGNORED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated ICCP overlimit missing W_META_ICCP_IGNORED"
    exit 0
}

test "${diag_line#*W_META_ICCP_SIZE_LIMIT_IGNORED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated ICCP overlimit missing W_META_ICCP_SIZE_LIMIT_IGNORED"
    exit 0
}

echo "ok" 1 - "builtin animated ICCP overlimit emits size-limit ignored contract"
exit 0

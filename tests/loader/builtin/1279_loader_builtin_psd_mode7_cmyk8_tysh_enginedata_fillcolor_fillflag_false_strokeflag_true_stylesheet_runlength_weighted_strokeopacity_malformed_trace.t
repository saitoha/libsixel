#!/bin/sh
# Verify malformed weighted StrokeOpacity run is ignored per-run and decode continues.
...
input_psd="${base_dir}/snake16_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillflag_false_strokeflag_true_stylesheet_runlength_weighted_strokeopacity_malformed.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode malformed weighted StrokeOpacity fixture: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    ...
    *)
        echo "not ok" 1 - "malformed StrokeOpacity trace is missing for malformed weighted run"
        exit 0
        ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "malformed weighted StrokeOpacity decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "malformed weighted StrokeOpacity keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

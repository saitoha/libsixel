...
input_psd="${base_dir}/snake16_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_dual_scope_stylesheet_array_color_values_precedence.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode TySh stylesheet-array /Color /Values precedence fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "TySh stylesheet-array /Color /Values precedence decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD TySh EngineData stylesheet-array /Color /Values precedence keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

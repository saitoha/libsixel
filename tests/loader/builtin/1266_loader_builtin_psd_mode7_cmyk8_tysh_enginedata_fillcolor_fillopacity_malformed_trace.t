#!/bin/sh
# Verify PSD mode7(4ch->CMYK8) TySh malformed FillOpacity is ignored with deterministic trace.
...
input_psd="${base_dir}/snake16_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillopacity_malformed.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 CMYK8 TySh malformed FillOpacity fixture: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    ...
    *)
        echo "not ok" 1 - "malformed FillOpacity trace is missing"
        exit 0
        ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "mode7 CMYK8 TySh malformed FillOpacity decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD mode7 CMYK8 TySh malformed FillOpacity keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

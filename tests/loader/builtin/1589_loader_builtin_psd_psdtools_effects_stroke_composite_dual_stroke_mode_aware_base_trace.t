...
case "${diag_line}" in
    *FX_DUAL_SOURCE_BASE*) ;;
    *)
        echo "not ok" 1 - "effects/stroke-composite missing FX_DUAL_SOURCE_BASE code"
        exit 0
        ;;
esac
...
case "${diag_line}" in
    *FX_VECTOR_ONLY_PREFER*)
        echo "not ok" 1 - "effects/stroke-composite regressed to vector-only stroke preference"
        exit 0
        ;;
    *) ;;
esac

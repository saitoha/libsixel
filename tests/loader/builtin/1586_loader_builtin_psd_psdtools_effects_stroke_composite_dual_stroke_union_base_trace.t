#!/bin/sh
# Verify base fallback path emits dual-stroke union coverage trace.
...
test "${trace_output#*builtin PSD: applying vector stroke and layer effect stroke in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite did not keep base vector/effect trace"
    exit 0
}
...
echo "ok" 1 - "effects/stroke-composite keeps base dual-stroke union contract"
exit 0

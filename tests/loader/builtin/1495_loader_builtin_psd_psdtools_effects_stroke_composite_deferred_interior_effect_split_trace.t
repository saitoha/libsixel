#!/bin/sh
# Verify deferred interior effects keep per-effect apply paths in clbl=1.
...
test "${trace_output#*builtin PSD: applying bevel shadow in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite deferred interior missed bevel shadow"
    exit 0
}
...
echo "ok" 1 - "effects/stroke-composite keeps deferred interior effect split non-regression contract"
exit 0

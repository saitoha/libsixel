...
case "${diag_line}" in
    *FX_BEVEL_SHADOW_APPLY*)
        echo "not ok" 1 - \
            "effects/stroke-composite deferred interior applied inactive bevel shadow"
        exit 0
        ;;
    *) ;;
esac
...
case "${diag_line}" in
    ...
    *)
        echo "not ok" 1 - \
            "effects/stroke-composite lost deferred stroke while applying deferred interior effects"
        exit 0
        ;;
esac

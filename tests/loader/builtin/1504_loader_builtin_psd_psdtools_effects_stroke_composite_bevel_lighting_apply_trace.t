...
test "${trace_output#*builtin PSD: parsed bevel lighting semantics*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite missed bevel lighting parse semantics"
    exit 0
}

test "${trace_output#*builtin PSD: parsed bevel highlight channel in layer effects*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite missed bevel highlight channel parse"
    exit 0
}

test "${trace_output#*builtin PSD: parsed bevel shadow channel in layer effects*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite missed bevel shadow channel parse"
    exit 0
}

#!/bin/sh
# Verify Lab ZIP path keeps ICC-skip trace behavior.
...
tmp_psd=$(mktemp "${TMPDIR:-/tmp}/libsixel-psd-lab8-zip-icc-XXXXXX.psd")
...
planes = [
    bytes([0x40, 0x80, 0xC0, 0xFF]),  # L*
    bytes([0x80, 0x90, 0x70, 0xA0]),  # a*
    bytes([0x80, 0x60, 0xA0, 0x90]),  # b*
]
...
case "${trace_log}" in
    *"builtin PSD: skipping embedded ICC conversion for Lab custom decode path"*)
        echo "ok" 1 - "Lab ZIP ICC skip trace is preserved"
        ;;
    *)
        echo "not ok" 1 - "Lab ZIP ICC skip trace is missing"
        exit 0
        ;;
esac

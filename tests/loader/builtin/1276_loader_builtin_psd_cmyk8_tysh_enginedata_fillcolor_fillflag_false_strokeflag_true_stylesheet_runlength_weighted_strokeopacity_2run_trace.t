#!/bin/sh
# Verify PSD CMYK8 TySh StyleRun weighted StrokeOpacity keeps run-length weighted opacity.
...
#       PPM:tests/data/loader/builtin_expected/psd_snake16_multilayer_fill_soco_gray_opacity_050_expected.ppm
...
input_psd="${base_dir}/snake16_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_fillflag_false_strokeflag_true_stylesheet_runlength_weighted_strokeopacity_2run.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode CMYK8 TySh weighted StrokeOpacity fixture: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    *"builtin PSD: malformed TySh StrokeOpacity; ignoring"*)
        echo "not ok" 1 - "malformed StrokeOpacity trace must not appear for valid weighted StrokeOpacity"
        exit 0
        ;;
    *) ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "CMYK8 TySh weighted StrokeOpacity decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD CMYK8 TySh weighted StrokeOpacity keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

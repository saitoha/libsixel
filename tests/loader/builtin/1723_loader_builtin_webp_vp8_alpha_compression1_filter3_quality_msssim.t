...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! -o "${output_builtin}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "builtin VP8+ALPHA compression=1 filter=3 decode failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L libwebp! -o "${output_libwebp}" \
    "${input_webp}" >/dev/null || {
    echo "not ok" 1 - "libwebp VP8+ALPHA compression=1 filter=3 decode failed"
    exit 0
}
...
echo "ok" 1 - "builtin VP8+ALPHA compression=1 filter=3 quality keeps MS-SSIM >= 0.98"
exit 0

#!/bin/sh
# Verify FXPRI_INSIDE overlap keeps quality under single-pass responsibility.
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - \
        "effects/stroke-composite FXPRI overlap LSQA fell below ${lsqa_floor}"
    exit 0
}

: "${lsqa_msg}"

echo "ok" 1 - "effects/stroke-composite keeps FXPRI overlap quality"
exit 0

...
input_webp="${TOP_SRCDIR}/tests/data/inputs/formats/orientation_xmp_o6_icc_displayp3_alias_vp8_static_12x8.webp"
trace_output=''
diag_line=''
command_status=0
nl='
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin static XMP orientation+CMS coexist decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin static XMP orientation+CMS coexist missing LSXWEBP1"
    exit 0
}
...
test "${diag_line#*W_OK_VP8L_STATIC*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static XMP orientation+CMS coexist missing W_OK_VP8L_STATIC"
    exit 0
}

test "${diag_line#*W_META_XMP_APPLIED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static XMP orientation+CMS coexist missing W_META_XMP_APPLIED"
    exit 0
}

test "${diag_line#*W_META_XMP_CMS_APPLIED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static XMP orientation+CMS coexist missing W_META_XMP_CMS_APPLIED"
    exit 0
}

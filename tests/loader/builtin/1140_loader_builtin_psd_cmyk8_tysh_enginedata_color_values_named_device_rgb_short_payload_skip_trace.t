...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "short-component PSD TySh /Color /DeviceRGB payload decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "short-component PSD TySh /Color /DeviceRGB payload is skipped with deterministic trace and keeps MS-SSIM ${lsqa_floor}"
exit 0

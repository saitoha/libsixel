...
input_webp="${TOP_SRCDIR}/tests/data/inputs/formats/animated-lossy-8x8-2frame-min-xmp-icc-srgb.webp"
trace_output=''
diag_line=''
command_status=0
nl='
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin animated XMP CMS sRGB decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin animated XMP CMS sRGB missing LSXWEBP1 header"
    exit 0
}
...
test "${diag_line#LSXWEBP1\|rc=0\|kind=OK\|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS sRGB malformed success header"
    exit 0
}

test "${diag_line#*W_OK_ANIM*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS sRGB missing W_OK_ANIM"
    exit 0
}

test "${diag_line#*W_META_XMP_CMS_APPLIED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin animated XMP CMS sRGB missing W_META_XMP_CMS_APPLIED"
    exit 0
}

echo "ok" 1 - "builtin animated XMP CMS sRGB emits APPLIED contract"
exit 0

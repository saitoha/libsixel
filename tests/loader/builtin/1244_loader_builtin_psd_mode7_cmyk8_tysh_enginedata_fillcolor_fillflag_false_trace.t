#!/bin/sh
# Verify PSD mode7(4ch->CMYK8) TySh EngineData FillFlag=false skips synthetic fill render.
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 CMYK8 TySh FillFlag=false fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "mode7 CMYK8 TySh FillFlag=false decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD mode7 CMYK8 TySh FillFlag=false keeps skip trace and MS-SSIM ${lsqa_floor}"
exit 0

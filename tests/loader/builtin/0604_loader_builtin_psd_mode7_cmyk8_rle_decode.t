#!/bin/sh
# Verify builtin loader maps PSD Multichannel(4ch) to CMYK 8-bit RLE decode.
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_mode7_cmyk8_rle.psd"
reference_ppm="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_cmyk8_expected.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_mode7_cmyk8_rle_output.six"
lsqa_floor=${LSQA_MS_SSIM_FLOOR:-0.995}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "PSD mode7(4ch) CMYK 8-bit RLE decode failed"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    "${reference_ppm}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "PSD mode7(4ch) CMYK 8-bit RLE fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD mode7(4ch) CMYK 8-bit RLE keeps MS-SSIM ${lsqa_floor}"
exit 0

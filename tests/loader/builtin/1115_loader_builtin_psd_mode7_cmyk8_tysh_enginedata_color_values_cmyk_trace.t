#!/bin/sh
# Verify mode7 PSD TySh EngineData /Color << /Values [...] >> parsing:
# fallback should render non-pixel fill payload and keep deterministic trace.
...
input_psd="${base_dir}/snake16_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_color_values_cmyk.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 TySh EngineData /Color fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "mode7 TySh EngineData /Color decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "mode7 PSD TySh EngineData /Color /Values path keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

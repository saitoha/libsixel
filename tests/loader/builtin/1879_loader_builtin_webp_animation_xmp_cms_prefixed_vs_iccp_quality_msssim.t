#!/bin/sh
# TAP test comparing animated prefixed XMP CMS output against ICCP output.
# Pair: animated-lossy-8x8-2frame-min-xmp-icc-displayp3-alias-psprefix.webp vs
...
input_xmp="${TOP_SRCDIR}/tests/data/inputs/formats/animated-lossy-8x8-2frame-min-xmp-icc-displayp3-alias-psprefix.webp"
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L builtin! -ldisable \
    ...
    echo "not ok" 1 - "builtin animated prefixed XMP CMS quality decode failed"
    exit 0
}
...
echo "ok" 1 - "builtin animated prefixed XMP CMS quality keeps MS-SSIM >= 0.98"
exit 0

#!/bin/sh
# Verify PSB Lab16 missing-composite marker is diagnosed as malformed
# layer record table.
# Fixture generation commands:
#   python3 tests/data/inputs/formats/generate_psb_missing_composite_fixtures.py

set -eux

test "${HAVE_IMG2SIXEL-}" = 1 || {
    printf "1..0 # SKIP img2sixel is disabled in this build\n"
    exit 0
}

echo "1..1"
set -v

input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_psb_lab16_missing_composite_marker.psd"
trace_log=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" -o /dev/null 2>&1 || true)

case "${trace_log}" in
    *"builtin PSD: malformed layer record table"*)
        echo "ok" 1 - "PSB Lab16 missing-composite marker is malformed layer record table"
        ;;
    *)
        echo "not ok" 1 - "PSB Lab16 missing-composite malformed layer-record trace is missing"
        exit 0
        ;;
esac

exit 0

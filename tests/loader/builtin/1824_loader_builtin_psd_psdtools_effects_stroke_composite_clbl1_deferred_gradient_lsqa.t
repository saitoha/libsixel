#!/bin/sh
# Verify clbl=1 deferred gradient overlay keeps coverage/gate quality contract.
...
test "${trace_output#*builtin PSD: applying clip-weighted deferred gradient overlay in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite missing deferred gradient trace"
    exit 0
}
...
test "${trace_output#*builtin PSD: separating deferred gradient coverage source and clip gate in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite missing deferred gradient coverage/gate split trace"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - \
        "effects/stroke-composite deferred gradient LSQA fell below ${lsqa_floor}"
    exit 0
}

: "${lsqa_msg}"

echo "ok" 1 - "effects/stroke-composite keeps deferred gradient quality"
exit 0

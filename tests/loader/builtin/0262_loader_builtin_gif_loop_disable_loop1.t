#!/bin/sh
# TAP test: loop=disable ignores NETSCAPE loop1 and emits one pass.
...
trace_log=$(
    ...
    echo "not ok" 1 - "loop disable ignores NETSCAPE loop1 trace run failed"
    exit 0
}
...
test "${actual_sequence}" = "${expected_once}" || {
    echo "not ok" 1 - "loop disable ignores NETSCAPE loop1 sequence mismatch"
    exit 0
}

echo "ok" 1 - "loop disable ignores NETSCAPE loop1"

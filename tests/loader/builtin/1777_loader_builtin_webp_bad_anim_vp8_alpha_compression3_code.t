...
input_webp="${TOP_SRCDIR}/tests/data/corrupted/bad_anim_vp8_alpha_compression3.webp"
trace_output=''
diag_line=''
command_status=0
nl='
...
test "${command_status}" -ne 0 || {
    echo "not ok" 1 - "forced builtin loader bad_anim_vp8_alpha_compression3.webp unexpectedly succeeded"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "forced builtin loader bad_anim_vp8_alpha_compression3.webp missing LSXWEBP1 contract header"
    exit 0
}
...
test "${diag_line#LSXWEBP1\|rc=1\|kind=ERR\|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "forced builtin loader bad_anim_vp8_alpha_compression3.webp malformed error contract header"
    exit 0
}

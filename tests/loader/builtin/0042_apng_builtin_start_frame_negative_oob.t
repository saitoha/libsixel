...
# TAP test: builtin loader rejects an out-of-range APNG start frame.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" --env SIXEL_TRACE_TOPIC=encode_handoff,apng_decode,lifecycle --env "SIXEL_LOADER_ANIMATION_START_FRAME_NO=-999" \
    ...
    >/dev/null && {
    echo "not ok" 1 - "out-of-range negative start frame succeeded"
    exit 0
}

echo "ok" 1 - "builtin loader rejects negative out-of-range start frame"
exit 0

...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin static unknown unprefixed XMP CMS decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin static unknown unprefixed XMP CMS missing LSXWEBP1"
    exit 0
}
...
test "${diag_line#*W_META_XMP_CMS_IGNORED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static unknown unprefixed XMP CMS missing W_META_XMP_CMS_IGNORED"
    exit 0
}

test "${diag_line#*W_META_XMP_CMS_APPLIED*}" = "${diag_line}" || {
    echo "not ok" 1 - "builtin static unknown unprefixed XMP CMS unexpectedly emitted W_META_XMP_CMS_APPLIED"
    exit 0
}

echo "ok" 1 - "builtin static unknown unprefixed XMP CMS emits IGNORED contract"
exit 0

#!/bin/sh
# Verify FXPRI_INSIDE alpha-aware overlap keeps LSQA quality contract.
...
test "${trace_output#*builtin PSD: applying effect-priority dual-stroke overlap on clipped group*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite missing FXPRI overlap apply trace"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - \
        "effects/stroke-composite FXPRI alpha-aware LSQA fell below ${lsqa_floor}"
    exit 0
}

: "${lsqa_msg}"

echo "ok" 1 - "effects/stroke-composite keeps FXPRI alpha-aware LSQA contract"
exit 0

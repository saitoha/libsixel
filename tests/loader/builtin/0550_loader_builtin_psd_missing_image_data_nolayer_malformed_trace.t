#!/bin/sh
# Verify builtin PSD reports malformed image data when composite payload is absent
# and no layer records exist.

set -eux

test "${HAVE_IMG2SIXEL-}" = 1 || {
    printf "1..0 # SKIP img2sixel is disabled in this build\n"
    exit 0
}

echo "1..1"
set -v

input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/stbi_minimal_missing_image_data_nolayer.psd"
trace_log=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" -o /dev/null 2>&1 || true)

case "${trace_log}" in
    *"builtin PSD: malformed image data section"*)
        echo "ok" 1 - "missing image data without layers is malformed"
        ;;
    *)
        echo "not ok" 1 - "missing image data malformed trace is missing"
        exit 0
        ;;
esac

exit 0

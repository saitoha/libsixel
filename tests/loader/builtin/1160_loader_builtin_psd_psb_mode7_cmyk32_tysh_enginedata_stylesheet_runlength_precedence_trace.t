#!/bin/sh
# Verify PSB mode7 CMYK32 TySh EngineData StyleRun RunLengthArray precedence:
...
input_psd="${base_dir}/snake16_psb_mode7_cmyk32_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_stylesheet_runlength_precedence.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode PSB mode7 CMYK32 TySh StyleRun RunLengthArray precedence fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "PSB mode7 CMYK32 TySh StyleRun RunLengthArray precedence decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSB mode7 CMYK32 TySh EngineData StyleRun RunLengthArray precedence keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

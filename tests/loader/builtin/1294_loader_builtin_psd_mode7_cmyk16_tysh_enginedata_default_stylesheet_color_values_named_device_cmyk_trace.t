#!/bin/sh
# Verify PSD mode7 CMYK16 TySh /DefaultStyleSheet /Color /Values
# [/DeviceCMYK ...] decode.
...
input_psd="${base_dir}/snake16_mode7_cmyk16_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_default_stylesheet_color_values_named_device_cmyk.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode mode7 CMYK16 TySh /DefaultStyleSheet /Color DeviceCMYK fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "mode7 CMYK16 TySh /DefaultStyleSheet /Color DeviceCMYK decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD mode7 CMYK16 TySh /DefaultStyleSheet /Color DeviceCMYK keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

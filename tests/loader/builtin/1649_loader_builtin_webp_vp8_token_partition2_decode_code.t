#!/bin/sh
# TAP test confirming forced builtin loader decodes VP8 token partition=2.
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "forced builtin loader decodes VP8 token partition=2 decode failed"
    exit 0
}
...
test "${diag_line#*W_OK_VP8_STATIC*}" != "${diag_line}" || {
    echo "not ok" 1 - "forced builtin loader decodes VP8 token partition=2 missing W_OK_VP8_STATIC contract code"
    exit 0
}

echo "ok" 1 - "forced builtin loader decodes VP8 token partition=2 with stable contract codes"
exit 0

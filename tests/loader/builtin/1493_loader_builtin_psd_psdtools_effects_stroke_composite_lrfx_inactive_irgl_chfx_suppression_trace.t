...
case "${diag_line}" in
    ...
    *)
        echo "not ok" 1 - "effects/stroke-composite lost inactive IrGl parse"
        exit 0
        ;;
esac

case "${diag_line}" in
    *FX_CHFX_INACTIVE_PARSE*) ;;
    *)
        echo "not ok" 1 - "effects/stroke-composite lost inactive ChFX parse"
        exit 0
        ;;
esac
...
case "${diag_line}" in
    *FX_DEFERRED_INTERIOR_CLIP*)
        echo "not ok" 1 - \
            "effects/stroke-composite unexpectedly applied deferred interior"
        exit 0
        ;;
    *) ;;
esac

echo "ok" 1 - "effects/stroke-composite keeps lrFX inactive suppression"
exit 0

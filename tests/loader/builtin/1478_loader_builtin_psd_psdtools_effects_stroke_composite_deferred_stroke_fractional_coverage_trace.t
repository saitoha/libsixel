...
test "${trace_output#*builtin PSD: applying fractional silhouette coverage for deferred effect stroke in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - "effects/stroke-composite did not apply fractional silhouette coverage"
    exit 0
}

echo "ok" 1 - "effects/stroke-composite keeps deferred fractional stroke coverage contract"
exit 0

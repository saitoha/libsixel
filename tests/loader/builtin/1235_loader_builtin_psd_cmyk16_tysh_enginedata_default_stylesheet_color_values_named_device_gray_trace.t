#!/bin/sh
# Verify PSD CMYK16 TySh /DefaultStyleSheet /Color /Values [/DeviceGray ...] decode.
...
input_psd="${base_dir}/snake16_cmyk16_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_default_stylesheet_color_values_named_device_gray.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode CMYK16 TySh /DefaultStyleSheet /Color DeviceGray fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear \
    ...
    echo "not ok" 1 - "CMYK16 TySh /DefaultStyleSheet /Color DeviceGray decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "PSD CMYK16 TySh /DefaultStyleSheet /Color DeviceGray keeps deterministic trace and MS-SSIM ${lsqa_floor}"
exit 0

#!/bin/sh
# Verify builtin loader reconstructs RGB32 from a PSD layer-only file
...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_rgb32_missing_composite_single_layer.psd"
reference_ppm="${TOP_SRCDIR}/tests/data/loader/builtin_expected/psd_snake16_rgb32_expected.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_missing_composite_single_layer_rgb32_output.six"
lsqa_floor=${LSQA_MS_SSIM_FLOOR:-0.995}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L builtin! "${input_psd}" >"${output_sixel}" || {
    echo "not ok" 1 - "builtin loader failed to decode layer-only PSD RGB32"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    "${reference_ppm}" "${output_sixel}" 2>&1) || {
    echo "not ok" 1 - "layer-only PSD RGB32 fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "layer-only PSD RGB32 keeps MS-SSIM ${lsqa_floor} against expected PPM"
exit 0

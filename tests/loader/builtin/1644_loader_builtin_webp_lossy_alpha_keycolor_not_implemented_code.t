#!/bin/sh
# TAP test confirming forced builtin loader rejects lossy alpha keycolor WebP.
...
test "${command_status}" -ne 0 || {
    echo "not ok" 1 - "forced builtin loader rejects lossy alpha keycolor WebP unexpectedly succeeded"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "forced builtin loader rejects lossy alpha keycolor WebP missing LSXWEBP1 contract header"
    exit 0
}
...
test "${diag_line#LSXWEBP1|rc=1|kind=ERR|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "forced builtin loader rejects lossy alpha keycolor WebP malformed error contract header"
    exit 0
}

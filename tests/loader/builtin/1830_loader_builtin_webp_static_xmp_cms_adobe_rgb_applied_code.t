...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin static XMP CMS Adobe RGB decode failed"
    exit 0
}

diag_line=${trace_output#*LSXWEBP1\|}
test "${diag_line}" != "${trace_output}" || {
    echo "not ok" 1 - "builtin static XMP CMS Adobe RGB missing LSXWEBP1 header"
    exit 0
}
...
test "${diag_line#LSXWEBP1\|rc=0\|kind=OK\|codes=}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static XMP CMS Adobe RGB malformed success header"
    exit 0
}

test "${diag_line#*W_OK_VP8_STATIC*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static XMP CMS Adobe RGB missing W_OK_VP8_STATIC"
    exit 0
}

test "${diag_line#*W_META_XMP_CMS_APPLIED*}" != "${diag_line}" || {
    echo "not ok" 1 - "builtin static XMP CMS Adobe RGB missing W_META_XMP_CMS_APPLIED"
    exit 0
}

echo "ok" 1 - "builtin static XMP CMS Adobe RGB emits APPLIED contract"
exit 0

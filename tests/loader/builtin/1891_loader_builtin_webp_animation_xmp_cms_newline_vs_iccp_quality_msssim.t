#!/bin/sh
# TAP test comparing animated newline XMP CMS output against ICCP output.
# Pair: animated-lossy-8x8-2frame-min-xmp-icc-srgb-newline.webp vs
...
input_xmp="${TOP_SRCDIR}/tests/data/inputs/formats/animated-lossy-8x8-2frame-min-xmp-icc-srgb-newline.webp"
input_iccp="${TOP_SRCDIR}/tests/data/inputs/formats/animated-lossy-8x8-2frame-min-embedded-srgb-icc.webp"
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -S -L builtin! \
    ...
    echo "not ok" 1 - "builtin animated newline XMP CMS quality decode failed"
    exit 0
}
...
echo "ok" 1 - "builtin animated newline XMP CMS quality keeps MS-SSIM >= 0.98"
exit 0

...
output_sixel="${ARTIFACT_LOCAL_DIR}/psd_missing_composite_multilayer_nonpixel_output.six"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode non-pixel multi-layer PSD: ${trace_output}"
    exit 0
}
...
case "${trace_output}" in
    *"builtin PSD: ignoring non-pixel payload in layer fallback"*)
        ;;
    *)
        echo "not ok" 1 - "non-pixel ignore info trace is missing"
        exit 0
        ;;
esac
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "non-pixel multi-layer decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "non-pixel multi-layer payload is ignored with info trace and decode keeps MS-SSIM ${lsqa_floor}"
exit 0

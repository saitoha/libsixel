...
test "${trace_output#*builtin PSD: applying clip-weighted deferred effect stroke in layer fallback*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite lost clip-weighted deferred stroke contract"
    exit 0
}
...
test "${trace_output#*builtin PSD: applying mode-aware dual-stroke blend on clipped group*}" \
    != "${trace_output}" || {
    echo "not ok" 1 - \
        "effects/stroke-composite lost deferred mode-aware dual-stroke contract"
    exit 0
}

echo "ok" 1 - \
    "effects/stroke-composite keeps deferred stroke contract with overlay ownership split"
exit 0

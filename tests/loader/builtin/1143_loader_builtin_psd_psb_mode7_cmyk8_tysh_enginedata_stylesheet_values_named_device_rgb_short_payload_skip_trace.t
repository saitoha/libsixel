...
input_psd="${TOP_SRCDIR}/tests/data/inputs/formats/snake16_psb_mode7_cmyk8_missing_composite_multilayer_nonpixel_nopixel_tysh_enginedata_fillcolor_stylesheet_values_named_device_rgb_short_payload.psd"
...
test "${command_status}" -eq 0 || {
    echo "not ok" 1 - "builtin loader failed to decode short-component PSB mode7 TySh StyleSheetData /DeviceRGB payload fixture: ${trace_output}"
    exit 0
}
...
lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -W linear -b "MS-SSIM:${lsqa_floor}" \
    ...
    echo "not ok" 1 - "short-component PSB mode7 TySh StyleSheetData /DeviceRGB payload decode fell below MS-SSIM ${lsqa_floor}: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "short-component PSB mode7 TySh StyleSheetData /DeviceRGB payload is skipped with deterministic trace and keeps MS-SSIM ${lsqa_floor}"
exit 0

#!/bin/sh
# TAP test: gdk-pixbuf2 rejects out-of-range integer start-frame env values.
...
error_text="$(${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    echo "not ok 1 - gdk-pixbuf2 accepted out-of-range start-frame env"
    exit 0
}

test "${error_text#*SIXEL_LOADER_ANIMATION_START_FRAME_NO is out of range.}" != \
    "${error_text}" || {
    echo "not ok 1 - gdk-pixbuf2 out-of-range env error message mismatch"
    exit 0
}

echo "ok 1 - gdk-pixbuf2 rejects out-of-range start-frame env"
exit 0

#!/bin/sh
# TAP test: libtiff colormgmt parity for grayscale/img_tiff_grayscale_icc0_wp0_pc1_trc1.tiff
...
output_six="${ARTIFACT_LOCAL_DIR}/img_tiff_grayscale_icc0_wp0_pc1_trc1_libtiff.six"
...
run_img2sixel -Llibtiff:cms=1! "${input_tiff}" >"${output_six}" || {
    echo "not ok" 1 - "libtiff decode failed: grayscale/img_tiff_grayscale_icc0_wp0_pc1_trc1"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:0.999"     "${reference_six}" "${output_six}" 2>&1) || {
    echo "not ok" 1 - "grayscale/img_tiff_grayscale_icc0_wp0_pc1_trc1: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "libtiff colormgmt matches reference: grayscale/img_tiff_grayscale_icc0_wp0_pc1_trc1"
exit 0

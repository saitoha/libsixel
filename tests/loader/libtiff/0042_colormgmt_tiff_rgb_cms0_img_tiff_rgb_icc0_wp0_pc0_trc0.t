#!/bin/sh
# TAP test: libtiff colormgmt parity for rgb/img_tiff_rgb_icc0_wp0_pc0_trc0.tiff with cms=0
...
output_six="${ARTIFACT_LOCAL_DIR}/img_tiff_rgb_icc0_wp0_pc0_trc0_libtiff_cms0.six"
...
run_img2sixel -Llibtiff:cms=0! "${input_tiff}" >"${output_six}" || {
    echo "not ok" 1 - "libtiff decode failed with cms=0: rgb/img_tiff_rgb_icc0_wp0_pc0_trc0"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:0.999" \
    "${reference_six}" "${output_six}" 2>&1) || {
    echo "not ok" 1 - "rgb/img_tiff_rgb_icc0_wp0_pc0_trc0 (cms=0): ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "libtiff colormgmt matches reference with cms=0: rgb/img_tiff_rgb_icc0_wp0_pc0_trc0"
exit 0

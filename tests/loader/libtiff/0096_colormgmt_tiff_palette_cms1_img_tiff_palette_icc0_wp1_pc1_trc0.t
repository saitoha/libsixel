#!/bin/sh
# TAP test: libtiff colormgmt parity for palette/img_tiff_palette_icc0_wp1_pc1_trc0.tiff
...
output_six="${ARTIFACT_LOCAL_DIR}/img_tiff_palette_icc0_wp1_pc1_trc0_libtiff.six"
...
run_img2sixel -Llibtiff:cms=1! "${input_tiff}" >"${output_six}" || {
    echo "not ok" 1 - "libtiff decode failed: palette/img_tiff_palette_icc0_wp1_pc1_trc0"
    exit 0
}

lsqa_msg=$(set +xv; run_lsqa -m MS-SSIM -b "MS-SSIM:0.998"     "${reference_six}" "${output_six}" 2>&1) || {
    echo "not ok" 1 - "palette/img_tiff_palette_icc0_wp1_pc1_trc0: ${lsqa_msg}"
    exit 0
}

echo "ok" 1 - "libtiff colormgmt matches reference: palette/img_tiff_palette_icc0_wp1_pc1_trc0"
exit 0

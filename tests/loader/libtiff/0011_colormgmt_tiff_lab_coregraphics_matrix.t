#!/bin/sh
# TAP test: libtiff CMS output tracks CoreGraphics TIFF Lab matrix references.
...
input_dir="${TOP_SRCDIR}/tests/data/colormgmt/input/tiff/lab"
reference_dir="${TOP_SRCDIR}/tests/data/colormgmt/reference/tiff/lab"
mkdir -p "${ARTIFACT_LOCAL_DIR}"

set -- "${input_dir}"/*.tiff
if [ "$1" = "${input_dir}/*.tiff" ]; then
    printf "1..0 # SKIP TIFF Lab colormgmt matrix fixture is missing\n"
    exit 0
fi
...
for input_tiff in "$@"; do
    ...
    echo "ok" "${index}" - "libtiff Lab colormgmt matrix matches reference: ${name}"
    index=$((index + 1))
done

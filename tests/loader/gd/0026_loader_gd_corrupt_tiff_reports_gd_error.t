#!/bin/sh
# TAP test: forced gd loader reports GD failure for malformed TIFF stream.
...
msg=$(set +xv; head -c 64 \
    ...
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L gd! -ldisable - \
    2>&1 >/dev/null) && {
    echo "not ok" 1 - "forced gd loader accepted malformed TIFF stream"
    printf '%s\n' '--- stderr ---' >&2
    printf '%s\n' "${msg}" >&2
    exit 0
}
...
echo "ok" 1 - "forced gd loader reports GD failure for malformed TIFF"
exit 0

#!/bin/sh
# TAP test for unsupported GD WBMP stdin path delegating to another loader.
...
test "${fallback_loader}" != "" || {
    printf "ok 1 # SKIP no fallback loader decoded WBMP stdin in this runtime\n"
    exit 0
}

msg=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L gd! -ldisable - \
    < "${input_wbmp}" 2>&1 >/dev/null) && {
    echo "not ok 1 - gd unexpectedly accepted unsupported WBMP stdin"
    exit 0
}

test "${msg#*runtime error: unable to decode input with available loaders*}" \
    != "${msg}" || {
    echo "not ok 1 - gd-only WBMP stdin failure missed generic decode error"
    printf '%s\n' "${msg}" >&2
    exit 0
}

test "${msg#*GD error*}" = "${msg}" || {
    echo "not ok 1 - gd-only WBMP stdin failure should not report GD error"
    printf '%s\n' "${msg}" >&2
    exit 0
}
...
trace_log=$(set +xv; SIXEL_LOADER_TRACE=1 ${SIXEL_RUNTIME-} \
    ...
    < "${input_wbmp}" 2>&1 >/dev/null) || {
    echo "not ok 1 - gd stdin fallback failed for unsupported WBMP"
    exit 0
}
...
echo "ok 1 - unsupported GD WBMP stdin path delegates to ${fallback_loader}"
exit 0

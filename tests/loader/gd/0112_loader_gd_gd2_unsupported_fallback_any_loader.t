#!/bin/sh
# TAP test for unsupported GD GD2 path delegating to another loader.
...
msg=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L gd! -ldisable \
    ...
    echo "not ok 1 - gd unexpectedly accepted unsupported GD2"
    exit 0
}

test "${msg#*runtime error: unable to decode input with available loaders*}" \
    != "${msg}" || {
    echo "not ok 1 - gd-only GD2 failure missed generic decode error"
    printf '%s\n' "${msg}" >&2
    exit 0
}

test "${msg#*GD error*}" = "${msg}" || {
    echo "not ok 1 - gd-only GD2 failure should not report GD error"
    printf '%s\n' "${msg}" >&2
    exit 0
}
...
trace_log=$(set +xv; SIXEL_TRACE_TOPIC=loader ${SIXEL_RUNTIME-} \
    ...
    "${input_gd2}" 2>&1 >/dev/null) || {
    echo "not ok 1 - gd fallback failed for unsupported GD2"
    exit 0
}
...
echo "ok 1 - unsupported GD GD2 path delegates to ${fallback_loader}"
exit 0

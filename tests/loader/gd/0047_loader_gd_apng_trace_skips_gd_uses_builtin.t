#!/bin/sh
# TAP test: APNG path skips gd and reaches builtin fallback with trace.
...
trace_log=$(set +xv; SIXEL_LOADER_TRACE=1 ${SIXEL_RUNTIME-} \
    ...
    2>&1 >/dev/null) || {
    echo "not ok" 1 - "gd,builtin APNG conversion failed"
    exit 0
}

test "${trace_log#*libsixel: trying builtin loader*}" != "${trace_log}" || {
    echo "not ok" 1 - "builtin loader was not attempted for APNG"
    exit 0
}

test "${trace_log#*libsixel: trying gd loader*}" = "${trace_log}" || {
    echo "not ok" 1 - "gd loader should be skipped for APNG input"
    exit 0
}

echo "ok" 1 - "APNG trace confirms gd skip and builtin fallback"
exit 0

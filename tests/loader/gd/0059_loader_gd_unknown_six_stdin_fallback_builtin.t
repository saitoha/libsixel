#!/bin/sh
# TAP test: gd delegates unknown .six from stdin to builtin fallback.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L gd! -ldisable - \
    ...
    echo "not ok" 1 - "gd unexpectedly accepted stdin .six input"
    exit 0
}
...
trace_log=$(set +xv; SIXEL_LOADER_TRACE=1 ${SIXEL_RUNTIME-} \
    ...
    2>&1 >/dev/null) || {
    echo "not ok" 1 - "gd,builtin fallback failed for stdin .six"
    exit 0
}

test "${trace_log#*libsixel: trying builtin loader*}" != "${trace_log}" || {
    echo "not ok" 1 - "builtin loader was not attempted for stdin .six"
    exit 0
}

test "${trace_log#*libsixel: trying gd loader*}" = "${trace_log}" || {
    echo "not ok" 1 - "gd loader should be skipped for stdin .six"
    exit 0
}

echo "ok" 1 - "gd delegates stdin .six and builtin fallback succeeds"
exit 0

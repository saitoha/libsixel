...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -Llibwebp:cms_engine=none! "${input_webp_noicc}" >"${output_noicc}" || {
    echo "not ok" 1 - "libwebp decode failed for non-ICC input (cms=0)"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -Llibwebp:cms_engine=none! "${input_webp_icc}" >"${output_icc}" || {
    echo "not ok" 1 - "libwebp decode failed for ICC input (cms=0)"
    exit 0
}

lsqa_msg=$(set +xv; ${SIXEL_RUNTIME-} "${LSQA_PATH}" -m MS-SSIM -b "MS-SSIM:0.999" "${output_noicc}" "${output_icc}" 2>&1) || {

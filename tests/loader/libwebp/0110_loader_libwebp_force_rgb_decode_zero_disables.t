#!/bin/sh
# TAP test: force-rgb value 0 keeps the default lossy YUV path.
...
case "${msg_false}" in
    *"static decode path=lossy_yuv "*)
        ;;
    *)
        echo "not ok" 1 - "force-rgb value 0 did not keep lossy_yuv path"
        printf '%s\n' '--- stderr ---' >&2
        printf '%s\n' "${msg_false}" >&2
        exit 0
        ;;
...
echo "ok" 1 - "force-rgb value 0 keeps the lossy YUV path"
exit 0

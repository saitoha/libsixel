#!/bin/sh
# TAP test: libwebp output-frame limit env ignores signed values.
...
msg=$(set +xv; SIXEL_TRACE_TOPIC=webp_decode SIXEL_LOADER_LIBWEBP_MAX_OUTPUT_FRAMES=-1 \
    ...
    2>&1 >/dev/null) || {
    echo "not ok" 1 - "signed output-frame limit unexpectedly failed decode"
    printf '%s\n' '--- stderr ---' >&2
    printf '%s\n' "${msg}" >&2
    exit 0
}
...
case "${msg}" in
    *"ignore invalid SIXEL_LOADER_LIBWEBP_MAX_OUTPUT_FRAMES=-1"*)
        echo "ok" 1 - "signed output-frame limit is ignored with trace"
        ;;
    *)
        echo "not ok" 1 - "expected signed-value ignore trace was missing"
        printf '%s\n' '--- stderr ---' >&2
        printf '%s\n' "${msg}" >&2
        exit 0
        ;;

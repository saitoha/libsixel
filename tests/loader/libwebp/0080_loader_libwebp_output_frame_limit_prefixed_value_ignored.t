...
msg=$(set +xv; SIXEL_TRACE_TOPIC=webp_decode SIXEL_LOADER_LIBWEBP_MAX_OUTPUT_FRAMES=' 1' \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -L libwebp! -ldisable "${image_webp}" \
    2>&1 >/dev/null) || {
    echo "not ok" 2 - "space-prefixed output-frame limit unexpectedly failed decode"
    printf '%s\n' '--- stderr ---' >&2
    printf '%s\n' "${msg}" >&2
    exit 0
}
...
case "${msg}" in
    *"ignore invalid SIXEL_LOADER_LIBWEBP_MAX_OUTPUT_FRAMES= 1"*)
        echo "ok" 2 - "space-prefixed output-frame limit is ignored with trace"
        ;;
    *)
        echo "not ok" 2 - "expected space-prefixed ignore trace was missing"
        printf '%s\n' '--- stderr ---' >&2
        printf '%s\n' "${msg}" >&2
        exit 0
        ;;

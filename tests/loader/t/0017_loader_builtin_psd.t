...
require_file "${psd_input}"

if run_img2sixel -Lbuiltin -v -w1 -h1 "${psd_input}" \
        >"${target_sixel}" 2>"${log_file}"; then
    if ! grep -q "libsixel: loader builtin succeeded" "${log_file}"; then
        fail ${case_id} "builtin PSD did not report success"
    elif ! check_sixel_output "${target_sixel}"; then
        fail ${case_id} "builtin PSD output content mismatch"
    else
        pass ${case_id} "builtin PSD conversion succeeds"
    fi
else
    fail ${case_id} "builtin PSD conversion failed"
fi

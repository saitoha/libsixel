 ...
 *
 * Verify that Jarvis, Judice & Ninke diffusion lets the retained 6delta color
 * compete with its palette lookup.  Threshold zero must still keep a retained
 * color that is at least as accurate as the palette entry.  The error against
 * that retained color must continue through the diffusion kernel.
 */
...
int
test_filter_0016_filter_dither_6delta_jajuni(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    sixel_dither_set_diffusion_type(dither, SIXEL_DIFFUSE_JAJUNI);
    ...
    if (indexes == NULL) {
        fprintf(stderr, "jajuni palette application failed\n");
        goto end;
    }
    if (indexes[0] != 0) {
        fprintf(stderr,
                "jajuni emitted palette index %u instead of 6delta keep\n",
                (unsigned int)indexes[0]);
        goto end;
    }
    /*
     * Pixel 130 selects palette 255, while retained 250 is closer.  The
     * retained error -120 and Jajuni's 7/48 weight produce 113 here;
     * diffusing the palette error -125 would produce 112 instead.
     */
    if (pixel[3] != 113u) {
        fprintf(stderr,
                "jajuni neighbor is %u instead of retained-error value 113\n",

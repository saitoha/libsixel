 ...
 *
 * Verify that Sierra Lite diffusion lets the retained 6delta color compete
 * with its palette lookup.  Threshold zero must still keep a retained color
 * that is at least as accurate as the palette entry.  The error against that
 * retained color must continue through the diffusion kernel.
 */
...
int
test_filter_0019_filter_dither_6delta_sierra1(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    sixel_dither_set_diffusion_type(dither, SIXEL_DIFFUSE_SIERRA1);
    ...
    if (indexes == NULL) {
        fprintf(stderr, "sierra1 palette application failed\n");
        goto end;
    }
    if (indexes[0] != 0) {
        fprintf(stderr,
                "sierra1 emitted palette index %u instead of 6delta keep\n",
                (unsigned int)indexes[0]);
        goto end;
    }
    /*
     * Pixel 130 selects palette 255, while retained 250 is closer.  The
     * retained error -120 and Sierra Lite's 1/2 weight produce 71 here;
     * diffusing the palette error -125 would produce 68 instead.
     */
    if (pixel[3] != 71u) {
        fprintf(stderr,
                "sierra1 neighbor is %u instead of retained-error value 71\n",

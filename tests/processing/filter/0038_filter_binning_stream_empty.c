 ...
 *
 * An all-transparent stream cannot produce a valid weighted-point artifact.
 * Preserve the resolved policy so the palette planner can choose a fallback.
 */
...
int
test_filter_0038_filter_binning_stream_empty(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    mask = (unsigned char *)sixel_allocator_malloc(allocator, 2u);
    if (mask == NULL) {
...
cleanup:
    ...
    if (SIXEL_FAILED(status)) {
        fprintf(stderr, "empty sample-stream binning contract failed\n");
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
}

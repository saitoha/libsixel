 ...
 *
 * Verify that Sierra Two-row diffusion lets the retained 6delta color compete
...
int
test_filter_0020_filter_dither_6delta_sierra2(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    sixel_dither_set_diffusion_type(dither, SIXEL_DIFFUSE_SIERRA2);
    ...
    if (indexes == NULL) {
        fprintf(stderr, "sierra2 palette application failed\n");
        goto end;
    }
    if (indexes[0] != 0) {
        fprintf(stderr,
                "sierra2 emitted palette index %u instead of 6delta keep\n",

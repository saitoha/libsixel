 ...
 *
 * Verify that Sierra-3 diffusion lets the retained 6delta color compete with
 * its palette lookup.  Threshold zero must still keep a retained color that
 * is at least as accurate as the palette entry.  The error against that
...
int
test_filter_0021_filter_dither_6delta_sierra3(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    sixel_dither_set_diffusion_type(dither, SIXEL_DIFFUSE_SIERRA3);
    ...
    if (indexes == NULL) {
        fprintf(stderr, "sierra3 palette application failed\n");
        goto end;
    }
    if (indexes[0] != 0) {
        fprintf(stderr,
                "sierra3 emitted palette index %u instead of 6delta keep\n",
                (unsigned int)indexes[0]);
        goto end;
    }
    /*
     * Pixel 130 selects palette 255, while retained 250 is closer.  The
     * retained error -120 and Sierra-3's 5/32 weight produce 111 here;
     * diffusing the palette error -125 would produce 110 instead.
     */
    if (pixel[3] != 111u) {
        fprintf(stderr,
                "sierra3 neighbor is %u instead of retained-error value 111\n",

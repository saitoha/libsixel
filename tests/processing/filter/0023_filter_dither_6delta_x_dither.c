 ...
 *
 * Verify that X-dither lets the retained 6delta color compete with the
...
int
test_filter_0023_filter_dither_6delta_x_dither(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    if (strength_env != NULL) {
        ...
        if (saved_strength == NULL) {
            fprintf(stderr, "X-dither strength save allocation failed\n");
            goto end;
        }
        memcpy(saved_strength, strength_env, saved_strength_size);
    }
    if (sixel_compat_setenv("SIXEL_DITHER_X_DITHER_STRENGTH", "1") != 0) {
        fprintf(stderr, "X-dither strength setup failed\n");
        goto end;
    }
    ...
    sixel_dither_set_diffusion_type(dither, SIXEL_DIFFUSE_X_DITHER);
    ...
    if (indexes[0] != 0 || result_value != 1) {
        fprintf(stderr,
                "X-dither did not record 6delta keep: index=%u mask=%d\n",
    ...
    if (strength_overridden != 0 && sixel_compat_setenv(
        ...
        fprintf(stderr, "X-dither strength restore failed\n");
        ok = 0;
    }
    free(saved_strength);

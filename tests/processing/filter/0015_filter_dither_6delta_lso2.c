 ...
 *
 * Verify that LSO2 lets the retained 6delta color compete with its palette
 * lookup.  Threshold zero must still keep a pixel when the displayed color is
 * at least as accurate as the palette entry LSO2 would emit.  The kept
 * pixel's error must continue through the LSO2 kernel to the next pixel.
 */
...
int
test_filter_0015_filter_dither_6delta_lso2(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    sixel_dither_set_diffusion_type(dither, SIXEL_DIFFUSE_LSO2);
    ...
    if (indexes == NULL) {
        fprintf(stderr, "LSO2 palette application failed\n");
        goto end;
    }
    if (indexes[0] != 0) {
        fprintf(stderr,
                "LSO2 emitted palette index %u instead of 6delta keep\n",
                (unsigned int)indexes[0]);
        goto end;
    }
    if (pixel[3] >= 130u) {
        fprintf(stderr,
                "LSO2 lost kept-pixel error before the next pixel: %u\n",

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r lanczos2 -w 120% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "lanczos2 upscale 120pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "lanczos2 upscale 120pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "lanczos2 upscale 120pct lsqa failed"

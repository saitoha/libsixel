#!/bin/sh
# Validate downscale 80pct scaling with lanczos2 resampling.
#
# Reference image details:
# - Source: tests/data/inputs/snake_64.png (64x64)
# - ImageMagick filter: Lanczos (lobes=2)
...
output_sixel="${ARTIFACT_LOCAL_DIR}/lanczos2-downscale_80pct.six"


${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r lanczos2 -w 80% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "lanczos2 downscale 80pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "lanczos2 downscale 80pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "lanczos2 downscale 80pct lsqa failed"

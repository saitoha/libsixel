#!/bin/sh
# Validate upscale 120pct scaling with gaussian resampling.
#
...
output_sixel="${ARTIFACT_LOCAL_DIR}/gaussian-upscale_120pct.six"


${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r gaussian -w 120% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "gaussian upscale 120pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "gaussian upscale 120pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "gaussian upscale 120pct lsqa failed"

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r nearest -w 80% -o "${output_sixel}" "${input_image}" || {
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "nearest downscale 80pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "nearest downscale 80pct lsqa failed"

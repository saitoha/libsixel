#!/bin/sh
# Validate downscale 80pct scaling with bilinear resampling.
#
...
output_sixel="${ARTIFACT_LOCAL_DIR}/bilinear-downscale_80pct.six"


${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r bilinear -w 80% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "bilinear downscale 80pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "bilinear downscale 80pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "bilinear downscale 80pct lsqa failed"

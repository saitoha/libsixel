...
lsqa_floor=${LSQA_MS_SSIM_FLOOR:-0.9780}
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r hanning -w 120% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "hanning upscale 120pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "hanning upscale 120pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "hanning upscale 120pct lsqa failed"

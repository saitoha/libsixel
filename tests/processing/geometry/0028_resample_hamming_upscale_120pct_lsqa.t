#!/bin/sh
# Validate upscale 120pct scaling with hamming resampling.
#
# Reference image details:
# - Source: tests/data/inputs/snake_64.png (64x64)
# - ImageMagick filter: Hamming (support=1)
...
output_sixel="${ARTIFACT_LOCAL_DIR}/hamming-upscale_120pct.six"


${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r hamming -w 120% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "hamming upscale 120pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-}" = "" || {
    echo "not ok" 1 - "hamming upscale 120pct lsqa failed"
    exit 0
}

echo "ok" 1 - "hamming upscale 120pct lsqa passed"
exit 0

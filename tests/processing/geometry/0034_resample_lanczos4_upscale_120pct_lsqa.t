#!/bin/sh
# Validate upscale 120pct scaling with lanczos4 resampling.
#
# Reference image details:
# - Source: tests/data/inputs/snake_64.png (64x64)
# - ImageMagick filter: Lanczos (lobes=4)
...
output_sixel="${ARTIFACT_LOCAL_DIR}/lanczos4-upscale_120pct.six"


${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -r lanczos4 -w 120% -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "lanczos4 upscale 120pct scaling failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "lanczos4 upscale 120pct lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "lanczos4 upscale 120pct lsqa failed"

...
test_gpudeq_0001_strtoul(int argc, char **argv)
{
    ...
    while (index < sizeof(cases) / sizeof(cases[0])) {
        if (sixel_compat_setenv("SIXEL_GPU_DEQUANT_THRESHOLD",
                                cases[index].text) != 0) {
            fprintf(stderr, "failed to set dequant threshold case %zu\n",
                    index + 1u);
            return EXIT_FAILURE;
        }
        actual = sixel_gpu_dequant_auto_threshold();
        if (actual != cases[index].expected) {
            fprintf(stderr,
                    "dequant threshold case %zu: got %zu, expected %zu\n",
                    index + 1u,
                    actual,
    ...
    (void)sixel_compat_setenv("SIXEL_GPU_DEQUANT_THRESHOLD", "");
    return EXIT_SUCCESS;
}

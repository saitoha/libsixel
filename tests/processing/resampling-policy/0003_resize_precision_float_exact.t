#!/bin/sh
# Compare resize_precision=float with a precomputed safe-tone PPM.
...
actual_sixel="${ARTIFACT_LOCAL_DIR}/precision-float-actual.six"
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" "$@" \
    -w3 -h1 -rbilinear \
    -jscalar:resize_precision=float \
    -o "${actual_sixel}" \
    "${fixture_root}/bilinear-precision-input.ppm" || {
    echo "not ok" 1 - "resize_precision=float failed"
    exit 0
}
${SIXEL_RUNTIME-} "${TEST_RUNNER_PATH}" \
    geometry/resampling_exact decode "${actual_sixel}" \
    "${fixture_root}/bilinear-precision-linear-expected.ppm" || {
    echo "not ok" 1 - "resize_precision=float pixels changed"
    exit 0
}

echo "ok" 1 - "resize_precision=float matches the safe-tone oracle"
exit 0

#!/bin/sh
# TAP test ensuring float32 STBN mask source differs from hash on animation.
...
test "${mask_output}" != "${hash_output}" || {
    echo "not ok" 1 - "float32 stbn-mask output matched stbn-hash"
    exit 0
}

echo "ok" 1 - "float32 stbn-mask output differs from stbn-hash"
exit 0

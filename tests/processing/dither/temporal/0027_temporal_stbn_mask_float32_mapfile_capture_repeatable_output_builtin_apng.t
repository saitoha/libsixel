#!/bin/sh
# TAP test ensuring float32 stbn-mask mapfile capture is repeatable.
...
first_capture_output=$(
    ...
    echo "not ok" 1 - "float32 stbn-mask first mapfile encode failed"
    exit 0
}
...
second_capture_output=$(
    ...
    echo "not ok" 1 - "float32 stbn-mask second mapfile encode failed"
    exit 0
}

test -s "${palette_output}" || {
    echo "not ok" 1 - "float32 stbn-mask mapfile output is empty"
    exit 0
}

test "${second_capture_output}" = "${first_capture_output}" || {
    echo "not ok" 1 - "float32 stbn-mask mapfile output is not repeatable"
    exit 0
}

echo "ok" 1 - "float32 stbn-mask mapfile output is repeatable"
exit 0

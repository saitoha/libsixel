...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d lso2:scan=serpentine -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "variable-coefficient LSO2 8-bit with serpentine scan lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "variable-coefficient LSO2 8-bit with serpentine scan lsqa passed"
    exit 0
}

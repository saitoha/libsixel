#!/bin/sh
# TAP test covering variable-coefficient LSO2 8-bit.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d lso2:scan=raster -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "variable-coefficient LSO2 8-bit lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "variable-coefficient LSO2 8-bit lsqa passed"
    exit 0
}

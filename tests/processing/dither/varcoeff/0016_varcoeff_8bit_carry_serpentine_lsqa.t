...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d lso2 -Y carry -y serpentine -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "variable-coefficient LSO2 8-bit with carry + serpentine scan lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "variable-coefficient LSO2 8-bit with carry + serpentine scan lsqa passed"
    exit 0
}

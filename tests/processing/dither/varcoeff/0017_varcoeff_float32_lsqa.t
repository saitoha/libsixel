#!/bin/sh
# TAP test covering variable-coefficient LSO2 float32.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d lso2:scan=raster -W oklab -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "variable-coefficient LSO2 float32 lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "variable-coefficient LSO2 float32 lsqa passed"
    exit 0
}

 ...
 *
 * Verify paired FS weights on the lookup 6delta keep path.
 */
...
int
test_perturb_0018_keep_lookup(int argc, char **argv)
{
    SIXELSTATUS status;
    ...
    status = sixel_dither_set_diffusion_perturb(dither, 0.5f);
    if (SIXEL_FAILED(status)) {
        goto end;
    }
    sixel_dither_perturb_init(&perturb, 0.5f, 0);
    sixel_dither_perturb_weights(&perturb, 0, 0, weights);
    expected = 130 + (-125 * weights[0] * 2 / 4096 + 1) / 2;
    ...
    if (indexes == NULL) {
        fprintf(stderr, "FS palette application failed\n");
        goto end;
    }
    if (indexes[0] != 0) {
        fprintf(stderr,
                "FS emitted palette index %u instead of 6delta keep\n",
                (unsigned int)indexes[0]);
        goto end;
    }
    if (pixel[3] != expected || pixel[4] != expected
            || pixel[5] != expected) {
        fprintf(stderr,
                "FS lost kept-pixel error before the next pixel: %u\n",

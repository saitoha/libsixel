...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d atkinson:scan=raster -W oklab -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed float32 Atkinson diffusion lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed float32 Atkinson diffusion lsqa passed"
    exit 0
}

#!/bin/sh
# TAP test covering fixed float32 Stucki diffusion.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d stucki:scan=raster -W oklab -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed float32 Stucki diffusion lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed float32 Stucki diffusion lsqa passed"
    exit 0
}

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d sierra:variant=3 -Y carry -y raster -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed 8-bit Sierra-3 with carry propagation lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed 8-bit Sierra-3 with carry propagation lsqa passed"
    exit 0
}

#!/bin/sh
# TAP test covering fixed 8-bit Sierra-3 diffusion.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d sierra:variant=3:scan=raster -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed 8-bit Sierra-3 diffusion lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed 8-bit Sierra-3 diffusion lsqa passed"
    exit 0
}

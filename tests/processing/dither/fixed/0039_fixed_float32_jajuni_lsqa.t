...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d jajuni:scan=raster -W oklab -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed float32 Jajuni diffusion lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed float32 Jajuni diffusion lsqa passed"
    exit 0
}

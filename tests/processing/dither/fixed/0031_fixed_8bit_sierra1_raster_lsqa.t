#!/bin/sh
# TAP test covering fixed 8-bit Sierra-1.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d sierra:variant=1:scan=raster -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed 8-bit Sierra-1 lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed 8-bit Sierra-1 lsqa passed"
    exit 0
}

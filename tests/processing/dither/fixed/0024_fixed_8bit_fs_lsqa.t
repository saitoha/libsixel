#!/bin/sh
# TAP test covering fixed 8-bit Floyd-Steinberg.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d fs:scan=raster -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "fixed 8-bit Floyd-Steinberg lsqa failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "fixed 8-bit Floyd-Steinberg lsqa passed"
    exit 0
}

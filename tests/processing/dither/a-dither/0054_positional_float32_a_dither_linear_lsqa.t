#!/bin/sh
# TAP test covering positional float32 A dither in linear space.
#
# Flow:
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -d a_dither:scan=raster -W linear -o "${output_sixel}" \
    "${input_image}" || {
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "positional float32 a_dither linear lsqa passed"
    exit 0
}

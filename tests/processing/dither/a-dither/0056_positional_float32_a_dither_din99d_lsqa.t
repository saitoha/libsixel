#!/bin/sh
# TAP test covering positional float32 A dither in DIN99d space.
#
# Flow:
...
run_img2sixel -d a_dither -y raster -W din99d -o "${output_sixel}" \
    "${input_image}" || {
    fail 1 "positional float32 a_dither din99d lsqa failed"
    exit 0
}
...
[ "${lsqa_run_status:-0}" -eq 0 ] && {
    pass 1 "positional float32 a_dither din99d lsqa passed"
    exit 0
}

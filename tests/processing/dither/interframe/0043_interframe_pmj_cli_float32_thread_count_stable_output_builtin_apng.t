#!/bin/sh
# TAP test ensuring float32 PMJ CLI output is stable across thread counts.
...
test "${multi_thread_output}" = "${single_thread_output}" || {
    echo "not ok" 1 - "float32 pmj cli output changed across thread counts"
    exit 0
}

echo "ok" 1 - "float32 pmj cli output is stable across thread counts"
exit 0

...
diag_line=${msg%%"${nl}"*}
test -n "${diag_line}" || {
    echo "not ok" 1 - "float32 source=pmj missing diagnostic header"
    exit 0
}
...
case "${diag_line}" in
    LSXDTH1\|rc=0\|*codes=*) ;;
    *)
        echo "not ok" 1 - "float32 source=pmj malformed diagnostic header"
        exit 0
        ;;
esac
...
case "${diag_line}" in
    *STRATEGY_SOURCE_PMJ*) ;;
    *)
        echo "not ok" 1 - "float32 source=pmj diagnostic code is missing"
        exit 0
        ;;
esac
...
case "${diag_line}" in
    ...
    *)
        echo "not ok" 1 - "float32 source=pmj missing input-boundary reset contract"
        exit 0
        ;;
esac
...
echo "ok" 1 - "float32 source=pmj resets between mixed-size inputs"
exit 0

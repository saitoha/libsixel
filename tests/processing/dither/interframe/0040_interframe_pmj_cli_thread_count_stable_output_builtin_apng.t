#!/bin/sh
# TAP test ensuring 8bit PMJ CLI output is stable across thread counts.
...
test "${multi_thread_output}" = "${single_thread_output}" || {
    echo "not ok" 1 - "8bit pmj cli output changed across thread counts"
    exit 0
}

echo "ok" 1 - "8bit pmj cli output is stable across thread counts"
exit 0

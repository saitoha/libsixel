#!/bin/sh
# TAP test ensuring internal STBN placeholder stays diffusion-equivalent.
...
stbn_output=$(
    SIXEL_DITHER_INTERFRAME_STRATEGY=stbn \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
        --threads=1 \
        -L builtin \
        -ldisable \
...
test "${stbn_output}" = "${diffusion_output}" || {
    echo "not ok" 1 - "stbn placeholder output differs from diffusion"
    exit 0
}

echo "ok" 1 - "stbn placeholder is diffusion-equivalent on builtin APNG"
exit 0

#!/bin/sh
# Validate SIXEL_SIMD_LEVEL=scalar path with scale+colorspace work and LSQA.
...
reference_image="${ARTIFACT_LOCAL_DIR}/snake_63_ref_scalar.png"
output_sixel="${ARTIFACT_LOCAL_DIR}/snake_63_scalar.six"
lsqa_run_status=0
lsqa_err=""
...
run_img2sixel --env SIXEL_SIMD_LEVEL=scalar -Wo -w63     -o "${output_sixel}" "${input_image}" || {
...
test "${lsqa_run_status}" -eq 0 && {
    pass 1 "SIXEL_SIMD_LEVEL=scalar reached MS-SSIM 0.99"
    exit 0
}
...
test "${lsqa_run_status}" -eq 0 && {
    pass 1 "SIXEL_SIMD_LEVEL=scalar reached MS-SSIM 0.98"
    exit 0
}

fail 1 "SIXEL_SIMD_LEVEL=scalar fell below MS-SSIM 0.98: ${lsqa_err}"
exit 0

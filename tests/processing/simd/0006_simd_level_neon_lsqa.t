#!/bin/sh
# Validate SIXEL_SIMD_LEVEL=neon path with scale+colorspace work and LSQA.
...
reference_image="${ARTIFACT_LOCAL_DIR}/snake_63_ref_neon.png"
output_sixel="${ARTIFACT_LOCAL_DIR}/snake_63_neon.six"
lsqa_run_status=0
lsqa_err=""
...
run_img2sixel --env SIXEL_SIMD_LEVEL=neon -Wo -w63     -o "${output_sixel}" "${input_image}" || {
...
test "${lsqa_run_status}" -eq 0 && {
    pass 1 "SIXEL_SIMD_LEVEL=neon reached MS-SSIM 0.99"
    exit 0
}
...
test "${lsqa_run_status}" -eq 0 && {
    pass 1 "SIXEL_SIMD_LEVEL=neon reached MS-SSIM 0.98"
    exit 0
}

fail 1 "SIXEL_SIMD_LEVEL=neon fell below MS-SSIM 0.98: ${lsqa_err}"
exit 0

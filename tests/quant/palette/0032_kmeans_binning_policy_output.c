...
test_palette_0032_kmeans_binning_policy_output(int argc, char **argv)
{
    ...
    static unsigned char const expected_alternate[3][12] = {
        {36, 64, 124, 176, 88, 58, 123, 181, 144, 191, 225, 153},
        {36, 64, 124, 176, 88, 58, 122, 163, 152, 177, 230, 144},
        {36, 64, 124, 162, 64, 6, 154, 173, 132, 168, 62, 16}
    };
    static unsigned char const expected_32bit[3][12] = {
        {36, 64, 124, 176, 88, 58, 123, 181, 144, 191, 225, 153},
        {35, 63, 124, 176, 88, 58, 122, 163, 152, 177, 230, 144},
        {36, 64, 124, 162, 64, 6, 154, 173, 132, 168, 62, 16}
    };
    SIXELSTATUS status;
    sixel_allocator_t *allocator;
    ...
    if (memcmp(actual, expected, sizeof(expected)) != 0
            /* Toolchains may round one half-way component upward. */
            && memcmp(actual,
                      expected_alternate,
                      sizeof(expected_alternate)) != 0
            /* 32-bit intermediates may round another midpoint downward. */
            && memcmp(actual, expected_32bit, sizeof(expected_32bit)) != 0
       ) {

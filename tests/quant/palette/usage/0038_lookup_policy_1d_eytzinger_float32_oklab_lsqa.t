...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" --lookup-policy=eytzinger --precision=float32 --working-colorspace=oklab -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "float32 Eytzinger OKLab colorspace conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "float32 Eytzinger OKLab colorspace lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "float32 Eytzinger OKLab colorspace lsqa failed"
exit 0

...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -t rgb -X linear -W linear -o "${output_sixel}" "${input_image}" || {
    echo "not ok" 1 - "img2sixel clustering linear working linear conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    echo "ok" 1 - "clustering linear working linear lsqa passed"
    exit 0
}
...
echo "not ok" 1 - "clustering linear working linear lsqa failed"

#!/bin/sh
# Verify GPL import accepts CR line endings.
...
actual_palette=$(printf "%s" "${actual_palette}" | tr -d '\015') || {
    echo "not ok" 1 - "CR GPL output normalization failed"
    exit 0
}

test "${actual_palette}" = "${expected_palette}" || {
    echo "not ok" 1 - "CR GPL entries changed"
    exit 0
}

echo "ok" 1 - "GPL import accepts CR line endings"
exit 0

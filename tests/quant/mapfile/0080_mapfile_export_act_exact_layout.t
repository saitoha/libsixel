#!/bin/sh
# Verify ACT export writes the complete canonical 772-byte layout.
...
cmp -s "${actual_palette}" "${expected_palette}" || {
    echo "not ok" 1 - "ACT palette layout changed"
    exit 0
}

echo "ok" 1 - "ACT export has exact entries, padding, count, and transparency"
exit 0

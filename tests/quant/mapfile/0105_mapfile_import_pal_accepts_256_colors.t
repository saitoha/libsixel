#!/bin/sh
# Verify JASC PAL import retains exactly 256 ordered entries.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -m pal-jasc:"${input_palette}" \
    ...
    echo "not ok" 1 - "256-color JASC PAL import failed"
    exit 0
}

cmp -s "${actual_palette}" "${input_palette}" || {
    echo "not ok" 1 - "JASC PAL did not retain 256 ordered entries"
    exit 0
}

echo "ok" 1 - "JASC PAL retains exactly 256 ordered entries"
exit 0

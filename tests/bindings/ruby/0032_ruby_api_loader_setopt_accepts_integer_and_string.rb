...
begin
  ...
  begin
    ...
    if Libsixel::API.failed?(status)
      puts 'not ok 1 - loader setopt rejected string payload for integer option id'
    else
      puts 'ok 1 - loader setopt accepts string payload for integer option id'
    end
  ensure
    Libsixel::API.sixel_loader_unref(loader) if loader && loader.to_i != 0
  end
rescue StandardError => e
  puts 'not ok 1 - loader setopt integer/string payload check failed'

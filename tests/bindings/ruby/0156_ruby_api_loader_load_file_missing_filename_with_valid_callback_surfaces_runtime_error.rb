...
begin
  ...
  begin
    status = Libsixel::API.sixel_loader_load_file(loader, '__missing__0156__.png', 0)
    if Libsixel::API.failed?(status)
      puts 'ok 1 - loader missing filename runtime error path verified'
    else
      puts 'not ok 1 - loader missing filename unexpectedly succeeded'
    end
  ensure
    Libsixel::API.sixel_loader_unref(loader) if loader && loader.to_i != 0
  end
rescue StandardError => e
  puts 'not ok 1 - loader missing filename runtime path check failed'

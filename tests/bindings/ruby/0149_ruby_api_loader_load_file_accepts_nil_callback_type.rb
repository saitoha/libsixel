...
begin
  ...
  begin
    status = Libsixel::API.sixel_loader_load_file(loader, source, nil)
    if Libsixel::API.failed?(status)
      puts 'ok 1 - loader load_file accepts nil callback type at wrapper boundary'
    else
      puts 'ok 1 - loader load_file accepts nil callback type at wrapper boundary'
    end
  rescue TypeError => e
    puts 'not ok 1 - loader load_file rejected nil callback type'
    puts "# #{e.class}: #{e.message}"
  ensure
    Libsixel::API.sixel_loader_unref(loader) if loader && loader.to_i != 0
  end
rescue StandardError => e
  puts 'not ok 1 - loader nil callback type acceptance check failed'
  puts "# #{e.class}: #{e.message}"
end

...
begin
  ...
  begin
    rejected = false
    begin
      status = Libsixel::API.sixel_loader_load_file(loader, source, 'not_callback')
    ...
    if rejected
      puts 'ok 1 - loader string callback rejection path verified'
    else
      puts 'not ok 1 - loader accepted string callback unexpectedly'
    end
  ensure
    Libsixel::API.sixel_loader_unref(loader) if loader && loader.to_i != 0
  end
rescue StandardError => e
  puts 'not ok 1 - loader string callback rejection check failed'

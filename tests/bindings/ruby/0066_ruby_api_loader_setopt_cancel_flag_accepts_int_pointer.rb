...
begin
  ...
  begin
    status = Libsixel::API.sixel_loader_setopt(
      loader,
      Libsixel::API::SIXEL_LOADER_OPTION_CANCEL_FLAG,
      Fiddle::Pointer.new(0)
    )
    if Libsixel::API.failed?(status)
      puts 'not ok 1 - loader setopt rejected cancel-flag null pointer'
    else
      puts 'ok 1 - loader setopt accepts cancel-flag null pointer'
    end
  ensure
    Libsixel::API.sixel_loader_unref(loader) if loader && loader.to_i != 0
  end
rescue StandardError => e
  puts 'not ok 1 - loader cancel-flag pointer path check failed'

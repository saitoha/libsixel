...
begin
  ...
  begin
    begin
      status = Libsixel::API.sixel_loader_load_file(loader, StringIO.new('dummy.png'), 0)
      if Libsixel::API.failed?(status)
        puts 'ok 1 - loader load_file rejects memoryview-like filename input'
      else
        puts 'not ok 1 - loader load_file accepted memoryview-like filename input'
      end
  ...
  puts 'not ok 1 - loader memoryview-like filename rejection check failed'

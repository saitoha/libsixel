...
begin
  ...
  begin
    status = Libsixel::API.sixel_loader_setopt(
      loader,
      Libsixel::API::SIXEL_LOADER_OPTION_REQCOLORS,
      '32'
    )

    if Libsixel::API.failed?(status)
      puts 'not ok 1 - loader setopt rejected numeric-string reqcolors input'
    else
      puts 'ok 1 - loader setopt accepts numeric-string reqcolors input'
    end
  ensure
    Libsixel::API.sixel_loader_unref(loader) if loader && loader.to_i != 0
  end
rescue StandardError => e
  puts 'not ok 1 - reqcolors numeric-string acceptance check failed'

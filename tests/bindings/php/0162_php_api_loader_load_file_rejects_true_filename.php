...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, true, function ($frame, $context): int {
            return 0;
        });
    ...
    if ($rejected) {
        echo "ok 1 - loader true filename rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted true filename unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader true filename rejection check failed\n";

...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $ffi->sixel_encoder_encode_bytes($encoder, $pixels, 1);
    } catch (Throwable $e) {
        $rejected = true;
    }
    ...
    if ($rejected) {
        echo "ok 1 - encoder encode_bytes requires height argument\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted missing height argument\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder missing-height argument check failed\n";

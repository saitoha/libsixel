...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $ffi->sixel_encoder_encode_bytes($encoder, $pixels);
    } catch (Throwable $e) {
        $rejected = true;
    }
    ...
    if ($rejected) {
        echo "ok 1 - encoder encode_bytes requires width argument\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted missing width argument\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder missing-width argument check failed\n";

...
try {
    ...
    $encoder = new Encoder();
    $encoder->setopt(Constants::SIXEL_OPTFLAG_INPUT, $source);
    $encoder->setopt(Constants::SIXEL_OPTFLAG_OUTPUT, $devNull);

    echo "ok 1 - encoder setopt accepts input and output flags\n";
} catch (Throwable $e) {
    echo "not ok 1 - encoder setopt input/output check failed\n";

...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $ffi->sixel_encoder_encode_bytes($encoder, $pixels, 1, 1);
    } catch (Throwable $e) {
        $rejected = true;
    }
    ...
    if ($rejected) {
        echo "ok 1 - encoder encode_bytes requires pixelformat argument\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted missing pixelformat argument\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder missing-pixelformat argument check failed\n";

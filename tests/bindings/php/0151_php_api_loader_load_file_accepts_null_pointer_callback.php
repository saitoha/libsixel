...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_loader_load_file($loader, '__missing__0151__.png', $nullLoad);

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "ok 1 - loader raw API accepts null-pointer callback sentinel type\n";
    } else {
        echo "not ok 1 - loader null-pointer callback sentinel unexpectedly succeeded\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader null-pointer callback-sentinel check failed\n";

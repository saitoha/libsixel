...
CDEF, $libPath);
    ...
    $loader = $out[0];
    $status = (int)$ffi->sixel_loader_setopt($loader, 8, 'builtin');

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "not ok 1 - loader setopt rejected string payload for integer option id\n";
    } else {
        echo "ok 1 - loader setopt accepts string payload for integer option id\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader setopt integer/string payload check failed\n";

...
CDEF, $libPath);
    ...
    $loader = $out[0];
    $status = (int)$ffi->sixel_loader_setopt($loader, 'loader-order', 'builtin');

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "ok 1 - loader setopt rejects non-numeric option identifier\n";
    } else {
        echo "not ok 1 - loader setopt accepted non-numeric option identifier\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader non-numeric option rejection check failed\n";

...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_encoder_encode_bytes($encoder, 'not-bytes', 1, 1, 0x03, null, 0);
    ...
    if ($rejected) {
        echo "ok 1 - encoder encode_bytes rejects string pixel buffer input\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted string pixel buffer input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder string buffer rejection check failed\n";

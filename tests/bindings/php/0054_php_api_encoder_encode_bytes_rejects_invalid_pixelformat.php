...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_encoder_encode_bytes($encoder, $pixels, 1, 1, -1, null, 0);

    $ffi->sixel_encoder_unref($encoder);
    $encoder = null;

    if (API::failed($status)) {
        echo "ok 1 - encoder encode_bytes rejects invalid pixelformat\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted invalid pixelformat\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder invalid-pixelformat rejection check failed\n";

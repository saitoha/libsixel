...
$status = (int)$ffi->sixel_output_new($out, function ($data, $size, $priv): int {
    throw new InvalidArgumentException('callback value error');
}, null, null);
if ($status !== 0) {
    exit(2);
}
...
try {
    ...
    if ($rc !== 0) {
        echo "ok 1 - output callback value error surfaces as child-process failure\n";
    } else {
        echo "not ok 1 - output callback value error did not surface\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output callback value-error propagation check failed\n";

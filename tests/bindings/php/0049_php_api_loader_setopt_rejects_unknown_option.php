...
CDEF, $libPath);
    ...
    $loader = $out[0];
    $status = (int)$ffi->sixel_loader_setopt($loader, 9999, '1');

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "ok 1 - loader setopt rejects unknown option id\n";
    } else {
        echo "not ok 1 - loader setopt accepted unknown option id\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader unknown-option rejection check failed\n";

...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_output_new($out, function ($chunk, $size, $priv) use (&$sawNullPriv): int {
        $sawNullPriv = ($priv === null) || (is_object($priv) && FFI::isNull($priv));
        return 0;
    }, null, null);
    ...
    if (!API::failed($status) && $sawNullPriv) {
        echo "ok 1 - output callback default priv is null when omitted\n";
    } else {
        echo "not ok 1 - output callback default priv behavior was unexpected\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output callback default-priv check failed\n";

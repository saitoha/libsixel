...
$child = <<<'CHILD'
class LocalCallbackBoom0148 extends RuntimeException {}
...
$status = (int)$ffi->sixel_output_new($out, function ($data, $size, $priv): int {
    throw new LocalCallbackBoom0148('typed boom');
}, null, null);
if ($status !== 0) {
    exit(2);
}
...
try {
    ...
    echo "not ok 1 - output callback custom-exception preservation check failed\n";

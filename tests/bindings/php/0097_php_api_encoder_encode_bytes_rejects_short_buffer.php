...
CDEF, $libPath);
    ...
    $nullPixels = $ffi->cast('unsigned char *', 0);
    $status = (int)$ffi->sixel_encoder_encode_bytes($encoder, $nullPixels, 2, 2, 0x03, null, 0);

    $ffi->sixel_encoder_unref($encoder);
    $encoder = null;

    if (API::failed($status)) {
        echo "ok 1 - encoder encode_bytes rejects null/short pixel buffer input\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted null/short pixel buffer input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder short-buffer rejection check failed\n";

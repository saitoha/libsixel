...
CDEF, $libPath);
    ...
    $bytearray = [98, 117, 105, 108, 116, 105, 110];
    $loaderOrder = pack('C*', ...$bytearray);

    $status = (int)$ffi->sixel_loader_setopt($loader, 8, $loaderOrder);

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "not ok 1 - loader rejected bytearray-like loader-order input\n";
    } else {
        echo "ok 1 - loader setopt accepts bytearray-like loader-order input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader-order bytearray-like acceptance check failed\n";

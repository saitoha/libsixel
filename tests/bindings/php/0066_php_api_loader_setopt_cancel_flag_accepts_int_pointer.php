...
CDEF, $libPath);
    ...
    $cancelFlag = $ffi->new('int[1]');
    $cancelFlag[0] = 0;

    $status = (int)$ffi->sixel_loader_setopt($loader, 7, $cancelFlag);

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "not ok 1 - loader setopt rejected cancel-flag integer pointer\n";
    } else {
        echo "ok 1 - loader setopt accepts cancel-flag integer pointer\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader cancel-flag pointer path check failed\n";

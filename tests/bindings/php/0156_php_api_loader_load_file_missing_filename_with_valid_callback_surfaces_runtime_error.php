...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_loader_load_file($loader, '__missing__0156__.png', function ($frame, $context): int {
        return 0;
    });
    ...
    if (API::failed($status)) {
        echo "ok 1 - loader missing filename with valid callback surfaces runtime-error status\n";
    } else {
        echo "not ok 1 - loader missing filename unexpectedly succeeded with valid callback\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader missing-filename runtime-error check failed\n";

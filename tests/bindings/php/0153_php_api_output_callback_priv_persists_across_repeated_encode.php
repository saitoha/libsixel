...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_output_new($out, function ($chunk, $size, $callbackPriv) use (&$calls, &$sawSamePriv, $ffi): int {
        $calls += 1;
        if ($callbackPriv === null || FFI::isNull($callbackPriv)) {
            $sawSamePriv = false;
            return 0;
        }
        $marker = $ffi->cast('int *', $callbackPriv);
        if ((int)$marker[0] !== 123) {
            $sawSamePriv = false;
        }
        return 0;
    ...
    if (!API::failed($first) && !API::failed($second) && $calls >= 2 && $sawSamePriv) {
        echo "ok 1 - output callback priv persists across repeated encode\n";
    } else {
        echo "not ok 1 - output callback priv persistence across repeated encode was unexpected\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output callback repeated-encode priv persistence check failed\n";

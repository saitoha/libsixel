...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_encoder_encode_bytes($encoder, $pixels, 1, 1, 0x03, 'bad-palette', 11);
    ...
    if ($rejected) {
        echo "ok 1 - encoder encode_bytes rejects string palette input\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted string palette input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder string palette rejection check failed\n";

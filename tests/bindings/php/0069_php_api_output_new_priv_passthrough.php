...
CDEF, $libPath);
    ...
    $privMarker = $ffi->new('int[1]');
    $privMarker[0] = 123;

    $writeCallback = function ($data, $size, $priv) use (&$seenChunks, &$callbackOk, $ffi): int {
        $seenChunks += 1;
        if ($priv !== null && !FFI::isNull($priv)) {
            $marker = $ffi->cast('int *', $priv);
            if ($marker[0] === 123) {
                $callbackOk = true;
            }
        }
    ...
    $status = (int)$ffi->sixel_output_new($out, $writeCallback, $privMarker, null);
    ...
    if (API::failed($status)) {
        echo "not ok 1 - sixel_encode failed with status {$status}\n";
    } elseif (!$callbackOk) {
        echo "not ok 1 - output callback priv marker was not preserved\n";
    } elseif ($seenChunks === 0) {
        echo "not ok 1 - output callback was not invoked\n";
    } else {
        echo "ok 1 - output callback receives priv payload from output constructor\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output callback priv passthrough check failed\n";

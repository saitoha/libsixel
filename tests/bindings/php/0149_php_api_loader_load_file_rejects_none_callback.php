...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_loader_load_file($loader, $source, null);

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "ok 1 - loader load_file rejects null callback\n";
    } else {
        echo "not ok 1 - loader load_file accepted null callback\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader null-callback rejection check failed\n";

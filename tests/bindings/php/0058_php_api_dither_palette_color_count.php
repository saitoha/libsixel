...
CDEF, $libPath);
    ...
    if ($count > 0) {
        echo "ok 1 - dither palette color count getter returned positive value ({$count})\n";
    } else {
        echo "not ok 1 - dither palette color count is not positive\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - dither palette color count check failed\n";

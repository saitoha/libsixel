...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $pathlike, function ($frame, $context): int {
            return 0;
        });
    ...
    if ($rejected) {
        echo "ok 1 - loader rejects path-like filename object without coercion\n";
    } else {
        echo "not ok 1 - loader accepted non-coercible path-like filename object\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader non-coercible path-like filename rejection check failed\n";

...
CDEF, $libPath);
    ...
    $bytearray = [100, 117, 109, 109, 121, 46, 112, 110, 103];

    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $bytearray, $loadCallback);
    ...
    if ($rejected) {
        echo "ok 1 - loader load_file rejects bytearray-like filename input\n";
    } else {
        echo "not ok 1 - loader load_file accepted bytearray-like filename input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader bytearray-like filename rejection check failed\n";

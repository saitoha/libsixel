...
$status = (int)$ffi->sixel_output_new($out, function ($data, $size, $priv): int {
    throw new RuntimeException('first boom');
}, null, null);
if ($status !== 0) {
    exit(2);
}
...
try {
    ...
    if (!is_resource($process)) {
        throw new RuntimeException('proc_open failed for failing output child');
    }
    fclose($pipes[0]);
    ...
    if (!is_resource($process)) {
        throw new RuntimeException('proc_open failed for succeeding output child');
    }
    fclose($pipes[0]);
    stream_get_contents($pipes[1]);
    fclose($pipes[1]);
    ...
    if ($failRc !== 0 && $successRc === 0) {
        echo "ok 1 - output callback failure is isolated across output handles\n";
    } else {
        echo "not ok 1 - output callback failure isolation per output handle diverged\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output callback failure-isolation check failed\n";

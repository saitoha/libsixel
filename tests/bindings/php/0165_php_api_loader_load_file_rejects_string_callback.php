...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $source, 'not_callback');
    ...
    if ($rejected) {
        echo "ok 1 - loader string callback rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted string callback unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader string callback rejection check failed\n";

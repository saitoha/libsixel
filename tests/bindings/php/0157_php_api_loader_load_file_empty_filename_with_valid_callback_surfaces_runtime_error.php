...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_loader_load_file($loader, '', function ($frame, $context): int {
        return 0;
    });
    ...
    if (API::failed($status)) {
        echo "ok 1 - loader empty filename with valid callback surfaces runtime-error status\n";
    } else {
        echo "not ok 1 - loader empty filename unexpectedly succeeded with valid callback\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader empty-filename runtime-error check failed\n";

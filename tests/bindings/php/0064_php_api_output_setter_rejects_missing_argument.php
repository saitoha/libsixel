...
CDEF, $libPath);
    ...
    try {
        $ffi->sixel_output_set_skip_header($output);
    } catch (Throwable $e) {
        $rejected = true;
    }
    ...
    if ($rejected) {
        echo "ok 1 - output setter rejects missing argument\n";
    } else {
        echo "not ok 1 - output setter accepted missing argument\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output setter missing-argument check failed\n";

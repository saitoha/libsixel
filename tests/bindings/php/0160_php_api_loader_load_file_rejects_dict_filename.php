...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, ['name' => 'dummy.png'], function ($frame, $context): int {
            return 0;
        });
    ...
    if ($rejected) {
        echo "ok 1 - loader dict filename rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted dict filename unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader dict-filename rejection check failed\n";

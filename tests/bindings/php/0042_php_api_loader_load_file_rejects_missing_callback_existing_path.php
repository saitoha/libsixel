...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_loader_load_file($loader, $source, $nullLoad);

    $ffi->sixel_loader_unref($loader);
    $loader = null;

    if (API::failed($status)) {
        echo "ok 1 - loader load_file rejects missing callback for existing path\n";
    } else {
        echo "not ok 1 - loader load_file accepted missing callback for existing path\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader missing-callback rejection check failed\n";

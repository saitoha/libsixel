...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, 12345, $loadCallback);
    ...
    if ($rejected) {
        echo "ok 1 - loader load_file rejects integer filename input\n";
    } else {
        echo "not ok 1 - loader load_file accepted integer filename input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader integer filename rejection check failed\n";

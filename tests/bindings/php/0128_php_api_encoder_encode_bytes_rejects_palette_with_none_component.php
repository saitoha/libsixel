...
CDEF, $libPath);
    ...
    $pixels = $ffi->new('unsigned char[1]');
    $pixels[0] = 0;

    $rejected = false;
    try {
        $status = (int)$ffi->sixel_encoder_encode_bytes($encoder, $pixels, 1, 1, 0x83, [255, null, 0], 3);
    ...
    if ($rejected) {
        echo "ok 1 - encoder encode_bytes rejects palette with null component\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted palette with null component\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - encoder null-palette-component rejection check failed\n";

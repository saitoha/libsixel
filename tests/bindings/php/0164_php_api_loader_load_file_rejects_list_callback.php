...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $source, []);
    ...
    if ($rejected) {
        echo "ok 1 - loader list callback rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted list callback unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader list callback rejection check failed\n";

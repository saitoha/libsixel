...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, 1.5, function ($frame, $context): int {
            return 0;
        });
    ...
    if ($rejected) {
        echo "ok 1 - loader float filename rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted float filename unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader float-filename rejection check failed\n";

...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $source, ['k' => 'v']);
    ...
    if ($rejected) {
        echo "ok 1 - loader dict callback rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted dict callback unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader dict callback rejection check failed\n";

...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $source, new stdClass());
    ...
    if ($rejected) {
        echo "ok 1 - loader load_file rejects non-callable callback\n";
    } else {
        echo "not ok 1 - loader load_file accepted non-callable callback\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader non-callable callback rejection check failed\n";

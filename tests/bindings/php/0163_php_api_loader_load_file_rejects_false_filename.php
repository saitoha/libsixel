...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, false, function ($frame, $context): int {
            return 0;
        });
    ...
    if ($rejected) {
        echo "ok 1 - loader false filename rejection path verified\n";
    } else {
        echo "not ok 1 - loader accepted false filename unexpectedly\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader false filename rejection check failed\n";

...
CDEF, $libPath);
    ...
    $warned = false;
    set_error_handler(function () use (&$warned): bool {
        $warned = true;
        return true;
    });
    $status = (int)$ffi->sixel_encoder_encode_bytes($encoder, $pixels, new stdClass(), 1, 0x03, null, 0);
    restore_error_handler();

    $ffi->sixel_encoder_unref($encoder);
    $encoder = null;

    if ($warned || API::failed($status)) {
        echo "ok 1 - encoder encode_bytes rejects object width path\n";
    } else {
        echo "not ok 1 - encoder encode_bytes accepted object width path\n";
    }
} catch (Throwable $e) {
    restore_error_handler();
    echo "not ok 1 - encoder object-width rejection check failed\n";

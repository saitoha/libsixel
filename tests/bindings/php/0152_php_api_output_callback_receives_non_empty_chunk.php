...
CDEF, $libPath);
    ...
    $status = (int)$ffi->sixel_output_new($out, function ($chunk, $size, $priv) use (&$sawNonEmpty): int {
        if ($size > 0 && strlen(FFI::string($chunk, $size)) > 0) {
            $sawNonEmpty = true;
        }
        return 0;
    }, null, null);
    ...
    if (!API::failed($status) && $sawNonEmpty) {
        echo "ok 1 - output callback receives non-empty payload chunk\n";
    } else {
        echo "not ok 1 - output callback chunk was unexpectedly empty\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - output callback non-empty chunk check failed\n";

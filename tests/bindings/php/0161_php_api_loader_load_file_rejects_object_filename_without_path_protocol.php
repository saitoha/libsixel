...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $filename, function ($frame, $context): int {
            return 0;
        });
    ...
    if ($rejected) {
        echo "ok 1 - loader object filename without path protocol is rejected\n";
    } else {
        echo "not ok 1 - loader accepted object filename without path protocol\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader object-filename rejection check failed\n";

...
CDEF, $libPath);
    ...
    $rejected = false;
    try {
        $status = (int)$ffi->sixel_loader_load_file($loader, $memoryviewLike, $loadCallback);
    ...
    if ($rejected) {
        echo "ok 1 - loader load_file rejects memoryview-like filename input\n";
    } else {
        echo "not ok 1 - loader load_file accepted memoryview-like filename input\n";
    }
} catch (Throwable $e) {
    echo "not ok 1 - loader memoryview-like filename rejection check failed\n";

...
my $ok = eval {
    ...
    die 'callback payload missing sixel introducer'
        if substr($payload, 0, 2) ne "\eP";
    Image::LibSIXEL::sixel_output_unref($output);
    Image::LibSIXEL::sixel_dither_unref($dither);
    1;
};

ok($ok, 'sixel_encode writes sixel payload through output callback');
diag($@) if !$ok && $@ ne '';

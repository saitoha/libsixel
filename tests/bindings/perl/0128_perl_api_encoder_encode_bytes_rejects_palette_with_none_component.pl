...
my $ok = eval {
    my $rc = system(
        ...
        . '$s=Image::LibSIXEL::sixel_encoder_encode_bytes($e,$pp,2,2,Image::LibSIXEL::Constants::SIXEL_PIXELFORMAT_PAL8(),"255,,0",6);'
        . 'Image::LibSIXEL::_sixel_encoder_unref($e);'
        . 'exit($s==Image::LibSIXEL::Constants::SIXEL_OK()?0:1);'
    );
    die 'encoder encode_bytes unexpectedly succeeded with undef palette component'
        if $rc == 0;
    1;
};

ok($ok, 'encoder encode_bytes rejects palette with undef component path');
diag($@) if !$ok && $@ ne '';

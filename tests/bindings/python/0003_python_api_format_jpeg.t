#!/bin/sh
# Verify Python bindings can encode JPEG input and emit a well-formed SIXEL stream.
...
output_path="${ARTIFACT_LOCAL_DIR}/JPEG.six"
python_output=$(env \
    LIBSIXEL_LIBDIR="${libdir}" \
    LD_LIBRARY_PATH="${libdir}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
    DYLD_LIBRARY_PATH="${libdir}${DYLD_LIBRARY_PATH:+:${DYLD_LIBRARY_PATH}}" \
    "${run_python}" - "${TOP_SRCDIR}/tests/data/inputs/snake_64.jpg" "${output_path}" 2>&1 <<'PY'
import pathlib
import sys

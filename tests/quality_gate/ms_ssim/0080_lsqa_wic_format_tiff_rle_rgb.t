...
test "${probe_status}" -eq 0 || skip_all "wic tiff rle rgb codec is unavailable"
...
run_img2sixel -Lwic! "${image_path}" >"${output_sixel}" || {
    fail 1 "wic tiff rle rgb conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    pass 1 "wic tiff rle rgb quality meets baseline"
    exit 0
}
...
fail 1 "wic tiff rle rgb quality regressed"

#!/bin/sh
# Verify WIC ICO RGBA decoding quality with an MS-SSIM baseline.
...
#   convert tests/data/inputs/formats/snake-64-reference-rgba.png tests/data/inputs/formats/snake-ico-rgba.ico
...
image_path="${TOP_SRCDIR}/tests/data/inputs/formats/snake-ico-rgba.ico"
...
test "${probe_status}" -eq 0 || skip_all "wic ico rgba codec is unavailable"

lsqa_floor=${LSQA_MS_SSIM_FLOOR_WIC_ICO_RGBA:-0.98}
...
output_sixel="${ARTIFACT_LOCAL_DIR}/wic_ico_rgba.six"
run_img2sixel -Lwic! "${image_path}" >"${output_sixel}" || {
    fail 1 "wic ico rgba conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    pass 1 "wic ico rgba quality meets baseline"
    exit 0
}
...
fail 1 "wic ico rgba quality regressed"

#!/bin/sh
# Verify WIC PNG RGBA decoding quality with an MS-SSIM baseline.
...
lsqa_floor=${LSQA_MS_SSIM_FLOOR_WIC_PNG_RGBA:-0.99}

printf '1..1\n'
set -v

image_path="${TOP_SRCDIR}/tests/data/inputs/formats/snake-64-reference-rgba.png"
reference_path="${TOP_SRCDIR}/tests/data/inputs/formats/snake-64-reference-rgb.png"
output_sixel="${ARTIFACT_LOCAL_DIR}/wic_png_rgba.six"
run_img2sixel -Lwic! "${image_path}" >"${output_sixel}" || {
    fail 1 "wic png rgba conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    pass 1 "wic png rgba quality meets baseline"
    exit 0
}
...
fail 1 "wic png rgba quality regressed"

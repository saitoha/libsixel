#!/bin/sh
# Verify WIC ICO palette decoding quality with an MS-SSIM baseline.
...
test "${probe_status}" -eq 0 || skip_all "wic ico palette codec is unavailable"

lsqa_floor=${LSQA_MS_SSIM_FLOOR_WIC_ICO_PALETTE:-0.98}
...
run_img2sixel -Lwic! "${image_path}" >"${output_sixel}" || {
    fail 1 "wic ico palette conversion failed"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    pass 1 "wic ico palette quality meets baseline"
    exit 0
}
...
fail 1 "wic ico palette quality regressed"

...
#   magick images/pngsuite/basic/basn6a08.png -background "#000" -alpha remove -alpha off -resize 32x -depth 8 \
...
if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0045_pngsuite_basic_width32_basn6a08_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "basic_width32 basic/basn6a08.png"
    else

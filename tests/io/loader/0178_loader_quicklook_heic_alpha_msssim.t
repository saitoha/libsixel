#!/bin/sh
# TAP test: quicklook decodes alpha HEIC with stable visual quality.
...
run_img2sixel --env SIXEL_THUMBNAILER_HINT_SIZE=64 -L quicklook! \
    ...
    >"${ARTIFACT_LOCAL_DIR}/quicklook_heic_alpha.six" || {
    echo "not ok" 1 "quicklook HEIC alpha decode failed"
    exit 0
}

lsqa_msg=$(run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" \
    "${TOP_SRCDIR}/tests/data/inputs/formats/snake-64-reference-rgba.png" \
    "${ARTIFACT_LOCAL_DIR}/quicklook_heic_alpha.six" 2>&1) || {
    echo "not ok" 1 "$lsqa_msg"
    exit 0
}

echo "ok" 1 "quicklook HEIC alpha decode preserves quality"
exit 0

...
run_img2sixel --env "SIXEL_LOADER_ANIMATION_START_FRAME_NO=-1" \
    ...
    fail 1 "coregraphics decode with negative start frame failed"
    exit 0
}
...
run_img2sixel --env "SIXEL_LOADER_ANIMATION_START_FRAME_NO=2" \
    ...
    fail 1 "coregraphics decode with equivalent positive frame failed"
    exit 0
}

cmp -s "${ARTIFACT_LOCAL_DIR}/coregraphics_start_negative.six" \
    "${ARTIFACT_LOCAL_DIR}/coregraphics_start_positive_equivalent.six" || {
    fail 1 "negative start frame did not map to last coregraphics frame"
    exit 0
}

pass 1 "coregraphics negative start frame resolves from tail"
exit 0

...
run_img2sixel -L gdk-pixbuf2! -ldisable \
    ...
    fail 1 "baseline gdk-pixbuf2 animation decode failed"
    exit 0
}
...
run_img2sixel --start-frame=1 \
    ...
    fail 1 "gdk-pixbuf2 decode with positive start frame failed"
    exit 0
}

cmp -s "${ARTIFACT_LOCAL_DIR}/gdk_start_default.six" \
    "${ARTIFACT_LOCAL_DIR}/gdk_start_positive.six" && {
    fail 1 "positive start frame did not change gdk-pixbuf2 output"
    exit 0
}

pass 1 "gdk-pixbuf2 positive start frame is applied"
exit 0

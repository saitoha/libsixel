...
run_img2sixel --env "SIXEL_LOADER_ANIMATION_START_FRAME_NO=1" \
    ...
    fail 1 "builtin GIF decode with positive start frame failed"
    exit 0
}

cmp -s "${ARTIFACT_LOCAL_DIR}/builtin_gif_start_default.six" \
    "${ARTIFACT_LOCAL_DIR}/builtin_gif_start_positive.six" && {
    fail 1 "positive start frame did not change builtin GIF output"
    exit 0
}

pass 1 "builtin GIF positive start frame is applied"
exit 0

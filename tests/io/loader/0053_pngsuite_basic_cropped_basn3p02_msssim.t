...
if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0053_pngsuite_basic_cropped_basn3p02_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "basic_cropped basic/basn3p02.png"
    else
        fail 1 "basic_cropped basic/basn3p02.png"
    fi
else
    fail 1 "basic_cropped basic/basn3p02.png"
fi

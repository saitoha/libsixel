...
#   magick images/pngsuite/background/bgai4a16.png -background "#fff" -alpha remove -alpha off -depth 8 \
#       -define ppm:format=plain PPM:tests/data/loader/pngsuite_expected/0069_pngsuite_background_white_bgai4a16_msssim.ppm
set -eux
...
input_png="${images_dir}/pngsuite/background/bgai4a16.png"
...
score_file="${ARTIFACT_LOCAL_DIR}/bgai4a16.ms_ssim.txt"
img2sixel_opts="-B#fff"

if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0069_pngsuite_background_white_bgai4a16_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "background_white background/bgai4a16.png"
    else
        fail 1 "background_white background/bgai4a16.png"
    fi
else
    fail 1 "background_white background/bgai4a16.png"
fi

#!/bin/sh
# TAP test: CoreGraphics decodes DDS DXT5 input when macOS supports DDS UTI.
...
run_img2sixel -L coregraphics! "${image_path}" >"${output_sixel}" || {
    fail 1 "coregraphics failed to decode DDS DXT5 input"
    exit 0
}
...
test "${lsqa_run_status:-0}" -eq 0 && {
    pass 1 "coregraphics decodes DDS DXT5 with expected quality"
    exit 0
}
...
fail 1 "coregraphics DDS DXT5 quality regressed"
exit 0

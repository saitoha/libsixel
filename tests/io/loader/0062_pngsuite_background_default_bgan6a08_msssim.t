...
#   magick images/pngsuite/background/bgan6a08.png -background "#000" -alpha remove -alpha off -depth 8 \
...
score_file="${ARTIFACT_LOCAL_DIR}/bgan6a08.ms_ssim.txt"
img2sixel_opts=""

if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0062_pngsuite_background_default_bgan6a08_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "background_default background/bgan6a08.png"
    else
        fail 1 "background_default background/bgan6a08.png"
    fi
else
    fail 1 "background_default background/bgan6a08.png"
fi

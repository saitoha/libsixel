...
#   magick images/pngsuite/background/bgwn6a08.png -background "white" -alpha remove -alpha off -depth 8 \
...
if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0066_pngsuite_background_default_bgwn6a08_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "background_default background/bgwn6a08.png"
    else
        fail 1 "background_default background/bgwn6a08.png"
    fi
else
    fail 1 "background_default background/bgwn6a08.png"
fi

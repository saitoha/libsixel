...
expected_ppm="${top_srcdir}/tests/data/loader/pngsuite_expected/0063_pngsuite_background_default_bgan6a16_msssim.ppm"
output_sixel="${ARTIFACT_LOCAL_DIR}/bgan6a16.sixel"
score_file="${ARTIFACT_LOCAL_DIR}/bgan6a16.ms_ssim.txt"
img2sixel_opts=""

if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0063_pngsuite_background_default_bgan6a16_msssim.ppm"
    exit "${status}"
fi

if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.87" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "background_default background/bgan6a16.png"
    else
        fail 1 "background_default background/bgan6a16.png"
    fi
else
    fail 1 "background_default background/bgan6a16.png"
fi

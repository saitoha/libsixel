...
#   magick images/pngsuite/basic/basn4a16.png -background "#000" -alpha remove -alpha off -crop 16x16+8+8 +repage -depth 8 \
...
score_file="${ARTIFACT_LOCAL_DIR}/basn4a16.ms_ssim.txt"
img2sixel_opts="-c16x16+8+8"

if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0057_pngsuite_basic_cropped_basn4a16_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "basic_cropped basic/basn4a16.png"
    else
        fail 1 "basic_cropped basic/basn4a16.png"
    fi
else
    fail 1 "basic_cropped basic/basn4a16.png"
fi

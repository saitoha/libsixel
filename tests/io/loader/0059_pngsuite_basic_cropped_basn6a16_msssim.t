...
expected_ppm="${top_srcdir}/tests/data/loader/pngsuite_expected/0059_pngsuite_basic_cropped_basn6a16_msssim.ppm"
...
if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0059_pngsuite_basic_cropped_basn6a16_msssim.ppm"
    exit "${status}"
fi

if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.80" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "basic_cropped basic/basn6a16.png"
    else
        fail 1 "basic_cropped basic/basn6a16.png"
    fi
else
    fail 1 "basic_cropped basic/basn6a16.png"
fi

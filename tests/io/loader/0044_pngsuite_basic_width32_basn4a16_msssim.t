...
#   magick images/pngsuite/basic/basn4a16.png -background "#000" -alpha remove -alpha off -resize 32x -depth 8 \
...
if [ ! -f "${expected_ppm}" ]; then
    fail 1 "missing expected image: 0044_pngsuite_basic_width32_basn4a16_msssim.ppm"
    exit "${status}"
fi
...
if run_img2sixel ${img2sixel_opts} "${input_png}" >"${output_sixel}"; then
    if run_lsqa -m MS-SSIM -b "MS-SSIM:0.98" "${expected_ppm}" - <"${output_sixel}" >"${score_file}"; then
        pass 1 "basic_width32 basic/basn4a16.png"
    else
        fail 1 "basic_width32 basic/basn4a16.png"
    fi
else
    fail 1 "basic_width32 basic/basn4a16.png"
fi

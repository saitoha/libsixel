#!/bin/sh
# Run lsqa checks for float32 1d-Eytzinger in the gamma colorspace.
# Quality floors tuned to 99% of the current lsqa metrics:
# - MS-SSIM floor: 0.976111
# - PSNR_Y floor: 37.241541
set -eux
...
output_sixel="${output_dir}/1d-eytzinger-float32-gamma.six"
output_png="${output_dir}/1d-eytzinger-float32-gamma.png"
...
PALETTE_LSQA_MS_SSIM_FLOOR=0.976111
PALETTE_LSQA_PSNR_FLOOR=37.241541
...
if run_img2sixel --lookup-policy=1d-eytzinger --precision=float32 \
    ...
    fail 1 "float32 1d-Eytzinger gamma colorspace conversion failed"
    exit "${status}"
fi
...
if palette_lsqa_assert_quality "${input_image}" "${output_png}" \
        "1d-eytzinger-float32-gamma" "${artifact_dir}"; then
    pass 1 "float32 1d-Eytzinger gamma colorspace lsqa passed"
else
    fail 1 "float32 1d-Eytzinger gamma colorspace lsqa failed"
fi

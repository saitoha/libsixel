#!/bin/sh
# Verify MS-SSIM for clustering linear colorspace selection.
#
# Flow summary:
# - Convert the input image with -X linear.
...
output_sixel="${artifact_dir}/cluster-linear.six"

require_file "${input_image}"

if run_img2sixel -t rgb -X linear -o "${output_sixel}"     "${input_image}"     2>>"${log_file}"; then
    :
else

...
output_sixel="${artifact_dir}/cluster-linear-work-oklab.six"

require_file "${input_image}"

if run_img2sixel -t rgb -X linear -W oklab -o "${output_sixel}"     "${input_image}"     2>>"${log_file}"; then
    :
else
    fail 1 "img2sixel clustering linear working oklab conversion failed"
    exit "${status}"
fi
...
if [ -z "${lsqa_run_status-}" ]; then
    pass 1 "clustering linear working oklab lsqa passed"
elif [ "${lsqa_run_status}" -eq 5 ]; then
    fail 1 "${lsqa_err}"
else
    fail 1 "clustering linear working oklab lsqa failed"
fi

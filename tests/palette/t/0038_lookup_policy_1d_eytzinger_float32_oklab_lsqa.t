...
output_sixel="${output_dir}/1d-eytzinger-float32-oklab.six"
output_png="${output_dir}/1d-eytzinger-float32-oklab.png"
...
if run_img2sixel --lookup-policy=1d-eytzinger --precision=float32 \
    ...
    fail 1 "float32 1d-Eytzinger OKLab colorspace conversion failed"
    exit "${status}"
fi
...
if lsqa_assert_quality "${input_image}" "${output_png}" \
        "1d-eytzinger-float32-oklab" "${artifact_dir}" "${lsqa_floor}"; then
    pass 1 "float32 1d-Eytzinger OKLab colorspace lsqa passed"
else
    fail 1 "float32 1d-Eytzinger OKLab colorspace lsqa failed"
fi

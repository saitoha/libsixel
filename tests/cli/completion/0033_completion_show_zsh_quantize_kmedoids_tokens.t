...
test "${msg#*auto*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids auto algo value in zsh completion"
    exit 0
}
...
test "${msg#*iter=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids iter key in zsh completion"
    exit 0
}

test "${msg#*sample=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids sample key in zsh completion"
    exit 0
}

test "${msg#*clara_trials=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids clara_trials key in zsh completion"
    exit 0
}

test "${msg#*clara_sample=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids clara_sample key in zsh completion"
    exit 0
}

test "${msg#*clarans_local=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids clarans_local key in zsh completion"
    exit 0
}

test "${msg#*clarans_neighbors=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids clarans_neighbors key in zsh completion"
    exit 0
}

test "${msg#*bandit_iter=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids bandit_iter key in zsh completion"
    exit 0
}

test "${msg#*bandit_candidates=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids bandit_candidates key in zsh completion"
    exit 0
}

test "${msg#*bandit_batch=*}" != "${msg}" || {
    echo "not ok" 1 - "missing kmedoids bandit_batch key in zsh completion"
    exit 0
}

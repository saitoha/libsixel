#!/bin/sh
# Verify cover snap rate through short and environment paths.
# Registry row: SIXEL_OPTION_SCHEMA_COVER_POLICY|NULL|snap_rate
# Registry binding: cover_policy_snap_approach_rate|cover_policy_snap_approach_rate_override
...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract,palette_contract \
    ...
    "-aauto:A0.5" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "snap rate short conversion failed"
    exit 0
}
test "${short_trace#*LSXSUB1|*key=snap_rate|stored=1|binding=cover_policy_snap_approach_rate,cover_policy_snap_approach_rate_override|value=0.5*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1|*key=snap_rate|stored=1|binding=cover_policy_snap_approach_rate,cover_policy_snap_approach_rate_override|value=0.5*}" != "${env_trace}" || {
...
test "${range_trace#*LSXSUB1|*key=snap_rate|stored=1|binding=cover_policy_snap_approach_rate,cover_policy_snap_approach_rate_override|value=1*}" != "${range_trace}" || {
...
echo "ok" 1 - "cover snap rate preserves image output"
exit 0

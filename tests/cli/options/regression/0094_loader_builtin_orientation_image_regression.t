#!/bin/sh
# Verify builtin:orientation preserves image output through short and env paths.
# Registry row: SIXEL_OPTION_SCHEMA_LOADERS|g_loader_values + SIXEL_LOADER_INDEX_BUILTIN|orientation
# Registry binding: builtin_enable_orientation
...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ...
    "-Lbuiltin:O0!" "${input_image}" 2>&1 >"${short_output}") || {
...
env_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_LOADER_BUILTIN_ORIENTATION=0" "-Lbuiltin!" \
...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "builtin:orientation short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "builtin:orientation image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "builtin:orientation preserves image output"
exit 0

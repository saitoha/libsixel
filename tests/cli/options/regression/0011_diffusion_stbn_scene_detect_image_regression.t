...
# Dither contract: scene_detect=1|scene_detect_override=1
...
test "${effect_trace#*LSXDTH1|*scene_detect=1|scene_detect_override=1*}" != "${effect_trace}" || {
    echo "not ok" 1 - "stbn scene detect did not reach the dither"
    exit 0
}
...
test "${short_trace#*LSXSUB1|*key=scene_detect|stored=1|binding=stbn_scene_detect_enabled,stbn_scene_detect_override*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1|*key=scene_detect|stored=1|binding=stbn_scene_detect_enabled,stbn_scene_detect_override*}" != "${env_trace}" || {

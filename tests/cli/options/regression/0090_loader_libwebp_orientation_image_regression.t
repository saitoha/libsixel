...
# Registry binding: libwebp_enable_orientation
...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ...
    "-Llibwebp:O0!" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "libwebp:orientation short conversion failed"
    exit 0
}

test "${short_trace#*LSXSUB1|*key=orientation|stored=1|binding=libwebp_enable_orientation*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1|*key=orientation|stored=1|binding=libwebp_enable_orientation*}" != "${env_trace}" || {

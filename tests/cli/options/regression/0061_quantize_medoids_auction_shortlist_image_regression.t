...
short_output="${artifact_dir}/0061-quantize-medoids-auction_shortlist-short-$$.six"
...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "medoids:auction_shortlist short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "medoids:auction_shortlist image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "medoids:auction_shortlist preserves image output"
exit 0

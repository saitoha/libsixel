#!/bin/sh
# Verify 6bit:shared_instance preserves image output through short and env paths.
# Registry row: SIXEL_OPTION_SCHEMA_LUT_POLICY|g_lookup_values + SIXEL_LOOKUP_BASE_6BIT|shared_instance
...
short_output="${artifact_dir}/0083-lookup-6bit-shared_instance-short-$$.six"
...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract,dither_contract \
    ...
    --threads=2 -p 16 "-~6bit:S1" "${input_image}" 2>&1 >"${short_output}") || {
...
test "${short_trace#*LSXDTH1|*shared=1|shared_override=1*}" != "${short_trace}" || {
    echo "not ok" 1 - "6bit shared_instance did not reach the dither"
    exit 0
}
...
test "${env_trace#*LSXDTH1|*shared=1|shared_override=1*}" != "${env_trace}" || {
    echo "not ok" 1 - "6bit environment shared_instance did not reach the dither"
    exit 0
}
...
test "${equivalence_status:-0}" -eq 0 || {
    echo "not ok" 1 - "6bit:shared_instance short and env output diverged"
    printf "# %s\n" "${equivalence_error}"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "6bit:shared_instance image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "6bit:shared_instance preserves image output"
exit 0

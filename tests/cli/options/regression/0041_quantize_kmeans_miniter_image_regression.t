...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ...
    -p 16 "-Qkmeans:U2" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "kmeans:miniter short conversion failed"
    exit 0
}

test "${short_trace#*LSXSUB1|*key=miniter|stored=1*}" != "${short_trace}" || {
...
env_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KMEANS_MINITER=2" -p 16 "-Qkmeans" \
    "${input_image}" 2>&1 >"${env_output}") || {
    echo "not ok" 1 - "kmeans:miniter env conversion failed"
    exit 0
}

test "${env_trace#*LSXSUB1|*key=miniter|stored=1*}" != "${env_trace}" || {

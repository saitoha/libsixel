...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "heckbert:merge_lloyd short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "heckbert:merge_lloyd image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "heckbert:merge_lloyd preserves image output"
exit 0

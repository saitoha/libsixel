#!/bin/sh
# Verify libjpeg:cms_engine preserves image output through short and env paths.
# Registry row: SIXEL_OPTION_SCHEMA_LOADERS|g_loader_values + SIXEL_LOADER_INDEX_LIBJPEG|cms_engine
# Registry binding: libjpeg_cms_engine
...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ...
    "-Llibjpeg:Ebuiltin!" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "libjpeg:cms_engine short conversion failed"
    exit 0
}

test "${short_trace#*LSXSUB1|*key=cms_engine|stored=1|binding=libjpeg_cms_engine*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1|*key=cms_engine|stored=1|binding=libjpeg_cms_engine*}" != "${env_trace}" || {
    echo "not ok" 1 - "cms_engine environment value was not stored"
    exit 0
}

cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "libjpeg:cms_engine short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "libjpeg:cms_engine image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "libjpeg:cms_engine preserves image output"
exit 0

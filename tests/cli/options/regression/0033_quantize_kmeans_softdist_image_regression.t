...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ...
    -p 16 "-Qkmeans:Bsoft:Dtrilinear" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "kmeans:softdist short conversion failed"
    exit 0
}

test "${short_trace#*LSXSUB1|*key=softdist|stored=1*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1|*key=softdist|stored=1*}" != "${env_trace}" || {

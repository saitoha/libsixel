...
test "${short_trace#*LSXSUB1\|*key=swap_topk\|stored=1\|binding=quantize_model_kcenter_swap_topk,quantize_model_kcenter_swap_topk_override\|value=2*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1\|*key=swap_topk\|stored=1\|binding=quantize_model_kcenter_swap_topk,quantize_model_kcenter_swap_topk_override\|value=2*}" != "${env_trace}" || {
...
width_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KCENTER_SWAP_TOPK=4294967296" \
    -p 16 "-Qcenter:Aswap" "${input_image}" 2>&1 >/dev/null) || {
    echo "not ok" 1 - "center:swap_topk width conversion failed"
    exit 0
}

test "${width_trace#*LSXSUB1\|*key=swap_topk\|stored=1}" = "${width_trace}" || {
    echo "not ok" 1 - "center swap_topk accepted an over-width value"
    exit 0
}

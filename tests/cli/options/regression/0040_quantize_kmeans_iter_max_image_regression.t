...
short_output="${artifact_dir}/0040-quantize-kmeans-iter_max-short-$$.six"
...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "kmeans:iter_max short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "kmeans:iter_max image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "kmeans:iter_max preserves image output"
exit 0

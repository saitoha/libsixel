...
short_output="${artifact_dir}/0056-quantize-medoids-histbits-short.six"
env_output="${artifact_dir}/0056-quantize-medoids-histbits-env.six"

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    "-Qmedoids:H3" "${input_image}" -o "${short_output}" || {
    echo "not ok" 1 - "medoids:histbits short conversion failed"
    exit 0
}

${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KMEDOIDS_HISTBITS=3" "-Qmedoids" \
...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "medoids:histbits short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "medoids:histbits image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "medoids:histbits preserves image output"
exit 0

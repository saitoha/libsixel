...
short_output="${artifact_dir}/0064-quantize-center-seed-short-$$.six"
env_output="${artifact_dir}/0064-quantize-center-seed-env-$$.six"

short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    -p 16 "-Qcenter:Aswap:S2" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "center:seed short conversion failed"
    exit 0
}
...
env_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KCENTER_SEED=2" -p 16 "-Qcenter:Aswap" \
    "${input_image}" 2>&1 >"${env_output}") || {
    echo "not ok" 1 - "center:seed env conversion failed"
    exit 0
}
...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "center:seed short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "center:seed image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "center:seed preserves image output"
exit 0

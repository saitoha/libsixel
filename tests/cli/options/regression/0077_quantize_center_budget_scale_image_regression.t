...
# Registry binding: quantize_model_kcenter_budget_scale|quantize_model_kcenter_budget_scale_override
...
short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ...
    -p 16 "-Qcenter:Dadaptive:Y1.4" "${input_image}" 2>&1 >"${short_output}") || {
    echo "not ok" 1 - "center:budget_scale short conversion failed"
    exit 0
}

test "${short_trace#*LSXSUB1|*key=budget_scale|stored=1|binding=quantize_model_kcenter_budget_scale,quantize_model_kcenter_budget_scale_override*}" != "${short_trace}" || {
    echo "not ok" 1 - "budget_scale short value was not stored"
    exit 0
}
...
test "${env_trace#*LSXSUB1|*key=budget_scale|stored=1|binding=quantize_model_kcenter_budget_scale,quantize_model_kcenter_budget_scale_override*}" != "${env_trace}" || {
    echo "not ok" 1 - "budget_scale environment value was not stored"
    exit 0
}

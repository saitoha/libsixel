...
env_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KMEDOIDS_BANDIT_CANDIDATES=1" \
    -p 16 "-Qmedoids:Abandit" \
...
range_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KMEDOIDS_BANDIT_CANDIDATES=4097" \
    -p 16 "-Qmedoids:Abandit" "${input_image}" 2>&1 >/dev/null) || {
    echo "not ok" 1 - "medoids:bandit_candidates upper conversion failed"
    exit 0
}
...
width_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KMEDOIDS_BANDIT_CANDIDATES=4294967296" \
    -p 16 "-Qmedoids:Abandit" "${input_image}" 2>&1 >/dev/null) || {
    echo "not ok" 1 - "medoids:bandit_candidates width conversion failed"
    exit 0
}

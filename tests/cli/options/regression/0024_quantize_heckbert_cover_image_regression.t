...
test "${short_trace#*LSXSUB1|*key=cover|stored=1|binding=quantize_model_cover,quantize_model_cover_override*}" != "${short_trace}" || {
...
test "${env_trace#*LSXSUB1|*key=cover|stored=1|binding=quantize_model_cover,quantize_model_cover_override*}" != "${env_trace}" || {
    echo "not ok" 1 - "cover environment value was not stored"
    exit 0
}

cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "heckbert:cover short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "heckbert:cover image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "heckbert:cover preserves image output"
exit 0

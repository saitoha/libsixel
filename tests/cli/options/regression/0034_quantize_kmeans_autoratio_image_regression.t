...
short_output="${artifact_dir}/0034-quantize-kmeans-autoratio-short-$$.six"
env_output="${artifact_dir}/0034-quantize-kmeans-autoratio-env-$$.six"

short_trace=$(set +xv; SIXEL_TRACE_TOPIC=suboption_contract \
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    -p 16 "-Qkmeans:Bauto:R1" "${input_image}" 2>&1 >"${short_output}") || {
...
cmp -s "${short_output}" "${env_output}" || {
    echo "not ok" 1 - "kmeans:autoratio short and env output differ"
    exit 0
}
...
test "${lsqa_status:-0}" -eq 0 || {
    echo "not ok" 1 - "kmeans:autoratio image quality regressed"
    printf "# %s\\n" "${lsqa_error}"
    exit 0
}

echo "ok" 1 - "kmeans:autoratio preserves image output"
exit 0

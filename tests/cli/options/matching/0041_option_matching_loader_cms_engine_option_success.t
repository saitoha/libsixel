#!/bin/sh
# TAP test verifying -#/--cms-engine maps to SIXEL_LOADER_CMS_ENGINE.
...
run_img2sixel \
    ...
    echo "not ok" 2 - "reference decode with env cms engine failed"
    exit 0
}
...
run_img2sixel \
    ...
    echo "not ok" 1 - "short --cms-engine option was rejected"
    echo "ok" 2 - "long --cms-engine option mirrors environment behavior"
    exit 0
}
...
run_img2sixel \
    ...
    echo "not ok" 2 - "long --cms-engine option was rejected"
    exit 0
}

cmp -s "${output_env}" "${output_long}" || {
    echo "not ok" 2 - "long --cms-engine output diverged from env reference"
    exit 0
}
echo "ok" 2 - "long --cms-engine option mirrors environment behavior"

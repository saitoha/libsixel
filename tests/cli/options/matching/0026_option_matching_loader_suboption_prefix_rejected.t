...
msg=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    ...
    -Lbuiltin:cms=auto! "${TOP_SRCDIR}/tests/data/inputs/snake_64.png" \
    -o/dev/null 2>&1) || status=$?

test "${status}" -eq 2 || {
    echo "not ok" 1 - "-L suboption key prefix exit status mismatch"
    exit 0
}
...
test "${diag_tail}" != "${diag_line}" || {
    echo "not ok" 1 - "-L suboption key prefix diagnostic mismatch"
    printf '%s\n' '--- stderr ---' >&2
    printf '%s\n' "${msg}" >&2
    exit 0
}

echo "ok" 1 - "-L suboption key prefix is rejected"
exit 0

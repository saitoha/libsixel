#!/bin/sh
# TAP test verifying libtiff -L suboptions are accepted.
...
probe_output=$(
    set +xv
    ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" -Llibtiff:cms_engine=none! \
...
case "${probe_output}" in
    *"invalid argument for -L,--loaders option"*)
        echo "not ok" 1 - "-L libtiff suboptions were rejected by option parser"
        printf '%s\n' '--- stderr ---' >&2
        printf '%s\n' "${probe_output}" >&2
        exit 0
        ;;

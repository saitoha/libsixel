#!/bin/sh
# TAP test verifying kmedoids clarans_neighbors accepts env/CLI values and keeps CLI priority.
...
${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    -Qkmedoids:clarans_neighbors=5000000 \
    "${TOP_SRCDIR}/tests/data/inputs/small.ppm" \
    -o/dev/null >/dev/null 2>&1 || {
...
msg=$(set +xv; ${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    --env "SIXEL_PALETTE_KMEDOIDS_CLARANS_NEIGHBORS=0" \
    -Qkmedoids:clarans_neighbors=5000001 \
    "${TOP_SRCDIR}/tests/data/inputs/small.ppm" \
    -o/dev/null 2>&1) && {
...
echo "ok" 1 - "kmedoids clarans_neighbors follows env/CLI acceptance and CLI priority"
exit 0

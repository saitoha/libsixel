...
# Verify a fixed number after use-macro suppresses invocation.
...
output=$(${SIXEL_RUNTIME-} "${IMG2SIXEL_PATH}" \
    -Lbuiltin! -7 -bgray2 -dnone -Efast -g -u -n7 \
    "${TOP_SRCDIR}/tests/data/inputs/formats/libpng-minimal-1x1-rgba.png") || {
    echo "not ok 1 - macro number after use-macro suppresses invocation: conversion failed"
    exit 0
}
...
test "$output" = "$expected" || {
    echo "not ok 1 - macro number after use-macro suppresses invocation: unexpected control stream"
    exit 0
}

echo "ok 1 - macro number after use-macro suppresses invocation"
exit 0
